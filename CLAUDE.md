...
3. An **anti-pattern naming block** inside that same self-check, calling out that upstream `SKILL.md` bodies routinely open with numbered "You MUST" checklists — and that reading such a checklist and starting to execute it *is* the failure mode this rule guards against. Without the explicit naming, the model treats two "MUST"s as equally authoritative and picks the one nearest to its action point (the upstream body, because it's inlined last).
