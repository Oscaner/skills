...
>
> **提交流程（G4③/P6 Task 17）:** 提交是脏树上发生的操作（`git add` + `git commit` 时工作树必有未提交改动），而 pre-commit 钩子结构性依赖树无关目标——`.husky/pre-commit` 运行 **`pnpm run precommit`**（即 `node scripts/run.ts precommit`），它由全量 validate 中**树无关子集**组成（`scripts/validate/pre-commit.ts`：emit freshness / osuperpowers 树 + wiring guard / engine zero residue + channel-audit / marketplace manifests / scripts unit / package version sync——6 组 9 块，**不含 engine 两块**（cdd-engine dev stub materialization / cdd-engine engine test suite），那两块以 `cwd=REPO_ROOT` 的真实序列 dispatch 依赖入口门放行，脏树即结构性失败点，已由 mkdtemp 隔离迁移到 infra `lifecycle.wiring.test.ts` 的独立干净临时仓）。**全量 11 块 validate（含树依赖黑盒）在 CI 干净检出上跑**（`.github/actions/validate` + `scripts/run.ts smoke-cdd`）——本地提交后如需完整断言面，先 commit 再跑 `pnpm run validate`。入口门保持真实边界不变。
