...
| `yaml` (eemeli) | `^2` (repo **root** devDependency) / 2.9.1 | YAML serialization | the hand-written `emitScalar` / `isPlainUnsafe` YAML builder (~50 lines) in the old report templates | see §2 (YAML isolation boundary) below |
