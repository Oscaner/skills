...
| # | Contract |
|---|---|
| C1 | Assembly order fixed — `[registry prefix] → [static template shell] → [variant payload all tailed]`; the variant payload (the `## Return` tail: the H1 four-line block / RETURN_FORMAT) never precedes the static shell. The shell's Handoff segment is param-driven — task number, round-suffixed handoff target, gate strings, findings/fixed-point paths are render-time slots of the static zone, not variant-tail content (segmentation note below) |
...
| C4 | Re-dispatch byte reuse — static zone memoized per (template × canonical static params); re-dispatches with byte-identical params (retries; consecutive implement rounds — buildCtx's implement fixedPoint is always `""` and implement.md declares no TASK_FIXED_POINT) hit the frozen shell, zero re-render. Round-dependent Handoff slots (fix/review handoff target, fixed point, findings path) change the memo key per round: those bytes re-render, while the shell invariant (title/context/instructions + Handoff layout) stays frozen |
| C5 | Dispatch-set constancy — same (harness, op, type) keeps byte-identical invoke string / model / cwd / env; a stable tree is a caching clause (dispatch never mutates the tree — the commit double-gate already enforces this). The engine never decides `model` (harness-side); the asserted byte-constant set is what the engine actually spawns: `{ cli, invoke-arg string, cwd, env }` — pinning this keeps the C4 memo's input precondition. This narrowing of the plan's C5 acceptance wording ("invoke 串/model/cwd/env") is recorded against that item and flagged for the T19 audit |
...
The T5 skeleton's static segments are title / context / instructions / **handoff** — every byte before the `## Return` heading. The **variant payload** is the `## Return` tail only (`RETURN_STDOUT_BLOCK` / `RETURN_FORMAT` content), and `{{VARIANT_TOKENS}}` resolves exclusively there (mechanically enforced by `validateTemplateStructure`). T5 deliberately keeps the Handoff inside the static shell, so round-suffixed handoff targets, task numbers, gate strings (`HANDOFF_WRITE_GATE` / `HANDOFF_TARGET`) and findings / fixed-point (`TASK_FINDINGS` / `TASK_FIXED_POINT` / `REVIEW_REFERENCE`) are **param slots of the static zone**, not variant-tail content. Their values change the canonical cache key (C3), so the C4 frozen-bytes guarantee is scoped to **byte-identical re-dispatches**: retries, and consecutive rounds whose params are identical — implement rounds in particular are byte-identical round to round (`buildCtx` implements a `""` fixed point and implement.md declares no `TASK_FIXED_POINT`). Consecutive fix/review rounds diverge at their round-dependent Handoff slots (handoff target line onward); the harness-side shared-prefix then covers the invariant title/context/instructions bytes only, and a `read > 0` flip must be recorded honestly against that prefix (C7), never claimed as whole-prompt reuse.
...
- Tooling: `node scripts/observe-cache.ts [--harness claude] [--rounds 2] [--cost|--debug] -- <workspace> <task> <mode>` runs ≥2 consecutive same-(harness, op, type) dispatch rounds with the measurement flag appended (default `--debug`; `--cost` is an explicit opt-in for harnesses that accept it) and extracts `{ readTokens, writeTokens }` per round (parse seam unit-tested at `scripts/__tests__/observe-cache.test.ts`). The run is measurement-only: no handoffs written, no workspace mutation — real rounds run through `cdd` (whose entry gate keeps the tree stable, C5). Fixed-point / findings params mirror `buildCtx`'s cross-phase read: implement rounds byte-identical; fix rounds read the same-round review handoff's `commits.base` when present, else document an approximation (see static-zone semantics).
