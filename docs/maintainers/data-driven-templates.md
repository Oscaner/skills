...
| review contract (P3) | `packages/cdd-engine/templates/template-contract.json#reviews` | `src/render/templates.ts` runtime (per-type config driving the shared docs/review.md shell) + `_docs/review.md` URC as the prose contract | cdd review / fix template rendering (runtime) | engine colocated tests (`src/render/__tests__/templates.*.test.ts`) + in-program single-source config |
...
| issue form yml (P4) | same `formFieldDefs` | `renderYml` in `scripts/render-yaml.mjs` (emit-only module — consumed via `scripts/emit/issue-templates.ts` wired into emitAll) | `.github/ISSUE_TEMPLATE/bug_report.yml` / `enhancement.yml` | `emit:check` drift + single-source `Object.keys` form-name assertion |
