...
| finding/report body (P4 — first runtime render) | `packages/osuperpowers/skills/report-issue/templates/finding-meta.json` | `packages/osuperpowers/scripts/report-templates.mjs` (renderYml / renderTitle / renderMeta / renderSummaryTable / renderComment / renderMasterBody pure functions) | `.github/ISSUE_TEMPLATE/*.yml` (emit) + finding comment / session master body (runtime) | `scripts/emit/issue-templates.test.mjs` two-stage round-trip + `emit:check` |
