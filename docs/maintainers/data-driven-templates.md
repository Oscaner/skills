...
| finding/report body (P4 — first runtime render) | `packages/osuperpowers/skills/report-issues/templates/finding-meta.json` | `packages/osuperpowers/scripts/report-templates.mjs` (renderYml / renderTitle / renderMeta / renderComment / renderMasterBody — renderTitle / renderComment retired by the §2.5 single-mode convergence, Task 14) | `.github/ISSUE_TEMPLATE/*.yml` (emit) + finding comment / session master body (runtime) | `scripts/emit/issue-templates.test.mjs` two-stage round-trip + `emit:check` |
...
> **First "one canonical, two-channel render" dogfood**: finding-meta.json drives both the emit product (issue form yml) and runtime products (report-issues finding comment / session master body) — P4 itself is the on-the-ground validation of this convention (AC12).
