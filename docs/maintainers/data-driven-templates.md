...
| review contract (P3) | `packages/cdd-engine/templates/review/reviews.json` | `src/render/templates.ts` runtime (per-type config driving the shared review.md shell) + `_docs/review.md` URC as the prose contract | cdd review / fix template rendering (runtime) | engine colocated tests (`src/render/__tests__/templates.*.test.ts`) + in-program single-source config |
