...
| `semver` | `^7` / 7.8.5 | version comparison | — (adopted pre-P5) | consumed by the repo-side version tooling (`scripts/lib/version-utils.ts`, version-sync validator); declared in cdd-engine dependencies but engine source does not import it yet |
