...
| `handlebars` | `^4` / 4.7.9 | template placeholder rendering (`{{X}}` syntax; strict compile; `{{{X}}}` triple-stash for schema/H1/brief injection) | the hand-written `{{PLACEHOLDER}}` split/join replace loops (`PLACEHOLDERS` array + `renderTemplate`) in the render module | ⚠️ **declared but not yet consumed** — the replace loops in `src/render/templates.mjs` are still live; the migration is pending. Do not drop the dependency (or the loops) until the render switch lands; anchor on `src/render/templates.mjs` |
...
| `execa` | `^9` / 9.6.1 | process spawning | — (adopted pre-P5) | `src/infra/proc.mjs` + `src/infra/invoke.mjs` — the engine's only spawn channel |
| `ajv` | `^8` / 8.20.0 | JSON schema validation | — (adopted pre-P5) | `src/rules/schema.mjs` / `schema.ts` — handoff JSON schema validation (task/docs) |
