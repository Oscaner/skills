...
- **Do**: ① **mismatch 单一推断模块**（engine rules 单点）：输入 = overall 解析（四表）+ 声明集 + engine 派生终态（终态源 = **该 parent overall 下所有 plan workspace**，非仅 dispatch 自身：`derivePlanVerdict.done`）；输出 = mismatch 集，分两门面——**结构性**（缺失 cell / 缺 claim）与**终态欠账**（plan-complete 未回填）；② **pre-flight 硬门接线（两门面皆硬门，v1.12）**——结构性面 mismatch 非空 → BLOCKED + 逐项指引（复用 Task 3 失败语义）；**终态欠账（plan-complete 未回填）→ plan-bearing dispatch（含 branch-review）BLOCKED + 指引「先 backfill-overall：version bump + claim + 列回填（branch-review 前置义务）」**；③ **lane 边界撤销（overall v1.12 用户裁决）**——branch 通道**不再排除**终态欠账成员（branch-review 前提 = plan 已结束 → 回填义务已到期 → 拦 = 正确行为）；docs 通道无 plan workspace → engine 终态声明源缺席 → 该成员天然 no-op（声明源缺席，非豁免；backfill 编辑路径恒放行）；④ **post-flight 高亮 recommand**——`statusValidate` 提升 base 默认（lanes 生效）；识别 plan-complete 且 mismatch 含「终态欠账」→ stdout 高亮打印下一步回填（可 diff 列态缺口、指向 overall 路径——Class B lineage 已知）；**exit 不变、fail-open**（触发窗口 = plan-done 至回填完成间所有 plan-bearing dispatch，含跨 phase 欠账）；⑤ **回填时序回归（v1.12）**——回填 = branch-review 前置义务：orchestration 先 backfill-overall（docs 编辑 overall + commit，经 docs 通道无 workspace 不自我挡）→ branch-review 放行；**未回填 → branch-review BLOCKED**（plan-done ∧ 未回填用例；已回填 → 放行 happy-path）；fresh checkout 无 progress.json → 零误伤；⑥ **finishing 撤销回填机制**——`packages/osuperpowers/skills/finishing/SKILL.md` backfill-overall 节点移除（finishing 仅存 merge/PR 决策 + close-issues）+ 派生产物 `pnpm run emit` 重生成 + 相关流程表述更新。
- **验收**: AC3——声明源 ↔ 列双向全列（forward + reverse，plan + design，无 plan-only 遗留）且 engine 派生终态并入声明源（终态源 = parent overall 下所有 plan workspace 枚举）；mismatch 集单一推断模块（改一处、pre/post 两通道行为同变——同源回归断言）；结构性 mismatch → BLOCK + 指引；**终态欠账 mismatch → pre-flight BLOCKED（含 branch-review 用例：plan-done ∧ 未回填 → BLOCKED；已回填 → 放行）+ post-flight 高亮回填 recommand（stdout 可捕获断言，exit 语义不变）**；AC2 部分（statusValidate 面）——task/docs/branch 三通道 statusValidate base 默认生效（含 post-flight 高亮单通道行为断言）；无死锁回归——回填时序前置（backfill 编辑路径不自我挡）+ fresh checkout 用例；**finishing SKILL.md 无 backfill-overall 残留（grep 断言）+ emit 产物零 drift**；engine 零文档写入（回填不落 engine 代码——设计约束断言）；engine vitest 就近全绿 + `pnpm run validate` 全绿（block 12 自检仍 2/2）。
- **注**: 本任务完成 closeout 两门面机械（结构性 + 终态欠账皆硬门）；AC7「无豁免例外常量」于此验证——lane 边界已撤销（v1.12 用户裁决），仅余声明源缺席语义（docs 无 plan workspace 天然 no-op，非常量）。**回填时序登记（v1.12）**：回填 = branch-review 前置义务——plan-done 后 orchestration 先执行 backfill-overall（version bump + claim + 四表列，docs 编辑 + commit；经 docs 通道无 workspace 不自我挡），branch-review pre-flight 时欠账已清 → 放行；未回填 → BLOCK 为正确行为（branch-review 前提 = plan 已结束）；finishing 撤销回填机制（backfill-overall 节点移除，仅存 merge/PR 决策 + close-issues），回填义务归 orchestration（本任务 ⑥ finishing SKILL.md 修订承载）。
