...
- **Version**: v1.5 · 2026-09-25（mid-flight 用户裁决：Skill 骨架结构从文本指导迁移为 canonical JSON Schema 单源——新增 **Task 11**（skill-anatomy schema + validate 机器校验 + 06-skill-authoring/CLAUDE.md 文本指导清理），plan 自 complete reopen 为 in-flight 继续展开）
- **Base**: develop
...
- **验收**:
  ...
  - 8 件 SKILL.md 经 skill-anatomy schema 机器校验全过（validate 绿）；**严格 allowlist**——现有 8 件标题面全 ∈ 注册表（对照通过），注册表外标题（如演进叙述段 `Flow size note`/`Full Flow Refactor Rationale`）→ BLOCK（engine 测试 mkdtemp 自造链故意破坏用例：删节点/断四要素/加未注册章节 → 全拦下，零本仓产物 fixture）
