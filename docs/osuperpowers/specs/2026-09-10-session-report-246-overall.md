...
单批次固定 date + slug 于 `docs/osuperpowers/`：
...
| P7 | submodule-bump workflow 精简：删除 5 步（find-issue / write-issue-body / create-issue / issue-number / comment-on-tracking-issue）+ PR 正文 "Tracking Issue:" 行；cpr 增 `delete-branch: true`（合并后自动删分支）；新增陈旧 PR reconcile（updated=false → 关闭未合并陈旧 PR + 删除分支 ref；updated=true 由 create-pull-request 内建 rebuild 复用）；docs/maintainers §Automated submodule sync 同步（删 Issue Action chain / `submodule:<name>` bootstrap / v1 迁移注记）；存量 #240/#241/#242 与 #117/#118 人工关闭存档 | P7-design（2026-09-13，v1.0） | Done | bump 时不再创建跟踪 issue；open issue 列表无 submodule-bump 自动杂音；无悬挂陈旧的 open bump PR 长期存在；存量 #240-242/#117/#118 已关闭 | 无 |
...
| v1.17 | 2026-09-13 | **P5 dev shipped**：CDD 八 task 全 APPROVED（engine 全权 artifact 写平面——`cdd base-branch set/get` 单源 + schema 校验 4 值 enum + 幂等 + CDD/standalone 双落点 + implement dispatch 自供应 brief（免 `cdd brief` 前置调用）+ slug 收敛（run-task+review 双派生点同源 trim `-design`/`-plan`）+ P4 守卫双形 glob + 代码目录/CLI 整理（删 bin 双 .gitkeep、cli shared 拆分、runBriefCli 归 lib）+ skills/docs 死说明收敛（dispatch-mode 删 brief 前置、determine-base/read-base 委托、base-branch.md enum 4 值 + 双 slug 拆分、review.md slug 同步））+ branch-review r1 APPROVED（findings 全 fix-inline）；changeset（osuperpowers minor + cdd-engine patch，显式 note：`cdd base-branch set/get` 可用、orchestrator 旧 `cdd brief` 前置调用兼容可删）+ `pnpm run emit`；Phase inventory P5 plan 列回填（[Pending]→Done）——F13 closeout 检查点执行（P5 守卫对自身五 phase ① 双向自证：本 closeout 行声明 plan Done ↔ 列同步一致）（v1.16→v1.17） | [human] · Claude Opus 5 (1M context) |
| v1.18 | 2026-09-13 | P7 design spec（`...-p7-design.md` v1.0）：F7/F8 同源「bump 产物生命周期缺回收闭环」——F7 删全部跟踪 issue 机制（5 步 + Tracking Issue 行 + `issues: write` 权限，bump 唯一载体=PR）+ F8 陈旧 PR reconcile（updated=false → close+删分支；updated=true 由 cpr 内建 rebase+force-push 覆盖，实证 owner-ref head 防 dependabot 误关）+ docs/maintainers §sync 同步 + 存量五条人工归档；spec-review r1 APPROVED（3 warn + 2 nit 全 fix，含 catch 收窄 404/422 / YAML parse acceptance / §3 同步口径补全）；Phase inventory P7 Design-spec 列回填（[Pending]→P7-design v1.0）+ scope 措辞精化（force-push 复用 → cpr 内建 rebuild 复用；reconcile close+删分支；删 5 步 incl write-issue-body + cpr delete-branch）；P5 closeout 补记：PR #254 已 merged（develop）（v1.17→v1.18） | [human] · Claude Opus 5 (1M context) |
| v1.19 | 2026-09-13 | **P7 dev shipped**：CDD 三 task 全 APPROVED（Task 1 `submodule-bump.yml` 重写——删 5 issue 步 + cpr `delete-branch: true` + reconcile 新步（owner-ref head + comment→close→deleteRef）；Task 2 sync 权限收缩 + maintainer doc 三处同步；Task 3 存量归档 #240/#241/#242/#117/#118 全 CLOSED + `chore/bump-impeccable` / `chore/bump-mattpocock-skills` 分支删除，纯 GH 运维无本地 commit）+ branch-review r1 APPROVED（1 warn fix-inline：reconcile audit comment 隔离 try/catch，故障经 core.warning 可观测且 close+deleteRef 恒执行 spec §2.6）——**无 changeset**（纯 workflow+maintainer 变更，无 npm package 影响）；Phase inventory P7 plan 列回填（[Pending]→Done）——F13 closeout 检查点执行（本 closeout 行声明 plan Done ↔ 列同步一致，P4 双向自证）（v1.18→v1.19） | [human] · Claude Opus 5 (1M context) |
