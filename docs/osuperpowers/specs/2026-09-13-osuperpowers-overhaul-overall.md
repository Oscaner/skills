...
| P5 | 用户 2026-09-17 补充（新 scope **engine 生命周期重建 + TS 化**） | **cdd-engine 全量 TypeScript 化 + 生命周期重建 + 框架收敛**（用户裁定：允许破坏性变更 · 重组代码/文件/目录 · 真注册面 · 抽象基类继承覆写 · 便于扩展 · 只维护功能逻辑；P6 的 engine lib 簇收口**前置到 P5**）：① **TS + unbuild**（全量 TS、抽象基类虚方法成为编译期约束；unbuild 构建/stub，dev 与发布同走 dist 入口）；② **CLI 换 citty**（unjs 生态，`node:util.parseArgs` 零 args 依赖、TS-first；替换 commander）；③ **生命周期域重建**——`DispatchLifecycle` 抽象基类（模板方法骨架 + hook 方法默认实现）· `TaskLifecycle`/`DocsLifecycle`（功能聚簇：自身 hook 全覆盖写，运维一文件看全）· **hookable 注册面**（`dispatch:before`/`dispatch:after` 固定 hook 点，外部插件注册；engine 内部变体走继承不依赖注册面）；④ **commit 边界双门**（入口门 pre-flight 干净树校验 dirty→BLOCKED + 出口门 post-flight validateCommitContract；run-task 增入口门、run-docs 接出口门、fix/docs.md 补提交指令）；⑤ **目录按功能重组**（cli / contract / git / handoff / lifecycle / runner / core / state）+ 命名统一（域目录复数、文件单数动词、hook 点 `域:动作`）；⑥ **开发调用链调整**（CLAUDE.md dev 段：`node packages/cdd-engine/bin/cdd.mjs` 随 bin 产品化作废 → `unbuild --stub` 后 `node packages/cdd-engine/dist/cli.mjs`，repo 工作树直调仍有效、**仍不 npm link 全局**）；⑦ 第三方收敛（v1.26 全项）+ **consola** 日志统一；engine 黑盒契约（4 子命令面 / handoff 输出 / failure 类目）零变化，400+ tests 为迁移护栏 |
