...
| P5 | 用户 2026-09-17 补充（新 scope **commit 边界纪律**） | **commit 边界纪律（双向不变量 + 归属原则）**：① **入口干净（pre-commit）**——派发任何 cdd review 前，主 agent 的产物（spec/plan/overall 回填）已全部提交；review 基准 = 已提交状态；主 agent dispatch 期间零写树（P4 两次 dirty-tree BLOCKED 的直接死因：dispatch 期间改树 → 返回时 commit-contract 判 dirty → handoff 改写 BLOCKED——入口干净 + 零写树 ⇒ 出口必然干净，永不重演）；② **出口干净（post-commit）**——任何产生文件修改的 dispatch 结束时修改已提交，**归属原则：「主 agent 处理 → 主 agent commit；cdd 处理 → cdd commit」**；③ **现状审计**：task implement/fix 模板已强制 agent 完成时 commit（conventional + 无 attribution + 无改动 skip）✓；**docs fix（spec/plan 的 fix）无 commit 指令 + `run-docs.mjs` "No commit-contract" —— 缺口**；review/branch-review 不产生修改无需 post-commit。落点：`fix/docs.md` 补 commit 指令（与 task 族同构：fix agent 完成时 commit 被修文档；无改动 skip）· `run-docs.mjs` 接 commit-contract 后验 · skills 流程描记 6 处加「review 前 pre-commit」+「fix 后 post-commit」· tests |
...
| v1.25 | 2026-09-17 | **P5 commit 边界纪律（用户 2026-09-17 新 scope，审计后定稿；Boundary rules）**：从「review 前/fix 后」两点直觉升维为**双向边界不变量 + 归属原则**——① 入口干净（pre-commit）：派发任何 cdd review 前主 agent 产物已全部提交，review 基准 = 已提交状态、主 agent dispatch 期间零写树 → 出口必然干净，**制度化消除 P4 两次 dirty-tree BLOCKED**（dispatch 期间改树 → commit-contract 判 dirty → handoff 改写 BLOCKED）；② 出口干净（post-commit）：产生修改的 dispatch 结束时修改已提交，归属「主 agent 处理 → 主 agent commit；cdd 处理 → cdd commit」；③ **现状审计确认**：task implement/fix 模板已强制 agent 完成时 commit ✓；**docs fix（`fix/docs.md` + `run-docs.mjs` "No commit-contract"）为真实缺口**；review/branch-review 无 post-commit 面。落点：`fix/docs.md` 补 commit 指令 · `run-docs.mjs` 接 validateCommitContract 后验 · 6 个 review-fix skill 加 review 前 pre-commit 描记 · tests。Requirement inventory 增行 + Phase inventory（P5 scope / acceptance）同步（v1.24→v1.25） | [human] · Claude Opus 5 (1M context) |
