...
| T7.7 | C-3 | **residue save 侧单点收敛（v1.18，T25 resume 实证）**——T25 续作把 T24 前草稿 `rules/residue.ts` 接入为 save side，与 T26 shipped `artifacts/residue.ts` 形成**同机制双 owner**（T24 收敛要消灭的形态）：双 stash-message 契约（`cdd residue:` vs 标准化）——legacy 检索不匹配；git helper 重复（gitStashPreserve vs gitStashPush）；task.ts 失败车道（artifacts）+ base 模板钩子（rules）同一 implement TIMEOUT 轮**双 stash**（residue_ref 后写覆盖、标准 stash 孤儿）。收敛 = save 全族**单 owner 归 artifacts/residue.ts**：`recoveryEligible`/`RESIDUE_PRESERVED_CAUSES` 迁入 · `preserved` 幂等守卫（已保全轮不二次 stash——template 钩子与车道共存时的防线）· `settleFromCarrier(cwd, handoffPath, repoRoot)` adapter（自 carrier 派生 op/type/task/round/cause，base 钩子 + branch 内联共用）· `preserveAndAnnounceResidue` announce 归位 · 删 `rules/residue.ts` + `gitStashPreserve` + 死 helper（gitDiffNumstat/gitUntrackedStat 无他引用时）· 全部 save 统一标准化 stash message（`cdd-<op>-<type>-task-N-rN-cause`——legacy 检索可恢复）· task.ts 车道保持直调（pre-write preserved→幂等跳过→恰一张 stash）· write-boundary（rules/write-boundary.ts）为新物保留不入本收敛 | artifacts/residue.ts · dispatch/base.ts · dispatch/branch.ts · dispatch/task.ts · infra/git.ts · rules/residue.ts（删）· rules/__tests__/rules.residue.test.ts（迁 artifacts）· tests |
| T7.8 | C-3/E2 | **引擎 lifecycle 统一校验（v1.19，用户三轮复盘定稿）**——closeout 暴露系统缺口（`[Pending]` 无义务状态：P6 行全程 pending、T14 任务行 pending 无显式可行动信号、物料格式无引擎判官）。统一抽象 = DispatchLifecycle 增**两个模板方法钩子**、共享一校验模块（§33 机制上链同律；实现/review/fix 每轮自动生效，零新增 CLI 面）：**① `docContractValidate`（pre-flight）**——物料必要格式校验（plan：`### Task N:` 连续可提取【brief 契约】· `**Spec:**` 引用存在 · constraints 源可提取 · 无占位；phase spec：Parent program 指向存在 overall · 目标 phase 已注册 · 版本行格式；overall：phase inventory canonical 表头 · **行形守卫**（复用已落地 cell 数一致性语义）· phase 行注册 · 版本升序 necessary 子集）→ 失败 **blocked exit + 指导信息**（每项缺什么/怎么修；退出码表 0/1/2/3 不变）· **② `statusValidate`（post-flight）**——六态收敛状态机（in-flight / needs-review / needs-fix / **needs-re-review**【review BLOCKED→fix APPROVED 无后续 review，T14 类显式化】/ resume-pending / complete，由任务 handoff 链派生）+ 计划完成度裁决（plan 任务集 ↔ 六态表）→ CDD_INFO 每轮输出（「plan Done」终态 = 引擎裁决消费，closeout 不再手排）。边界：引擎零文档写入（overall 为编排面）、零载体 schema 变更；scripts block-12 深层 maintainer 检查保留、necessary 子集同源不重复；validate 行形守卫（已落地）保留零债。 | dispatch/base.ts（两钩子挂载）·（新）rules/documents.ts 或 artifacts/documents-validate.ts（物料契约 + 六态派生 + 裁决共用模块）· render/brief.ts（plan 提取复用）· tests |
| T7.9 | C-3/E2 | **TaskState 单源统一 + 状态判定修正（v1.20，用户双发现）**——T29 首个 statusValidate 裁决暴露：**① 判定优先级错位**——deriveTaskState 按 counts 相等（reviews===fixes）判定、未以最后 review 状态为第一信号 → 合法终局（review-APPROVED → 终局 fix）全被误判 needs-re-review（T29 自身 fix-1 复现：28/29 全 needs-re-review）；**② TaskState 双源**——progress.json 存储 tasks[N].status（task.ts 写回）与 rules/status.ts 六态派生并存（T24 同机制两定义复发）。统一 = **状态纯派生、账本只存事实**：`deriveTaskState` 修正为唯一 TaskState 实现（最后 review APPROVED → complete【其后的 fix = 合法终局，非重审触发】· 最后 review 非 APPROVED + 后有 fix → needs-re-review【T14 类】/ 无 fix → needs-fix · 无 review → implement 车道：APPROVED→needs-review / BLOCKED→in-flight / TIMEOUT·EXEC_FAILURE→resume-pending）；**progress.json tasks[N].status 删除**（task.ts:900/903 写回降为「确保行在 + 计数」，不写 status；createEmptyProgress（progress.ts:213）不再 seed；readProgressJSON 迁移兼容旧行含 status——忽略）；全消费方走 deriveTaskState。 | rules/status.ts · artifacts/progress.ts（类型 + createEmptyProgress + read 迁移）· dispatch/task.ts（写回）· __tests__（status / progress-owner / progress / runner / lifecycle-validation）· tests |
| T7.10 | D-1 | **src 注释锚首清零执法（v1.21，用户发问）**——§35 第二半（src 注释语义主体 + 锚仅 trailing 溯源后缀）纸面无执法 → 引擎 67 处注释以阶段锚开头（`P6 T24 C:` / `T26:` / `Task 23:` / `spec T7.8:` 等，含 T29/T30 新注释）回归。修法：**① residue 守卫扩展**——新增「src 注释禁锚首」检查（`src/**/*.ts` 注释内容首 token ∈ 阶段锚 regex 族【P\d+|T\d+|Task \d+|spec T\d+】→ FAIL；文件头以路径开头合法、锚作 trailing 溯源后缀合法）· **② 排修**——67 处改为语义前置（锚移尾或删，每个注释语义不失真）· **③ 测试**——守卫新用例（合法 trailing 后缀绿 / 锚首红 / 文件头绿）+ 引擎全绿。注入面零锚首守卫（§35 第一半）不变。 | scripts/validate/residue.ts + __tests__ · packages/cdd-engine/src/**（67 注释排修） |
