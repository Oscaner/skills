...
- **Parent program**: [2026-09-13-osuperpowers-overhaul-overall.md v1.47](./2026-09-13-osuperpowers-overhaul-overall.md)（P6 行 scope/acceptance 已含全部 brainstorm 收敛 + v1.28–v1.47 全部登记）
...
| F8a | **H1 内容级 rename（v1.10，用户 2026-09-19「内容级 rename 没做完」指示）**：T20 只扫 token/zone 面——**prompt 正文 2 条 bullet 残留**（contract shell「from your H1 four lines」·「→ H1 `status: BLOCKED`」）+ **src 标识符/阶段标题**（`progress.ts` h1FourLines/h1FromHandoff/h1CountersLine · `finalize.ts` artifactsFromH1Line/implementStatusFromH1 · `phases.ts`「H1 four-line parse」· task.ts/exit.ts 散文注释）；rename 至 RETURN_STDOUT_BLOCK 语义名（returnFourLines/returnFromHandoff/returnCountersLine/artifactsFromReturnLine/implementStatusFromReturnLine/「return block parse」）；**`\bH1\b` 入 residue stale-lexicon 机制面守卫（零豁免）**——「H1 类历史名零残留」从命名约定升机械断言（§2.0 宣称≡现实）；验收 = prompt 正文 + src 零 `H1`/`h1` 机制标识符 | naming-conventions.md · template-contract.json#sections.shell · progress.ts · finalize.ts · phases.ts · residue |
