...
- **Version**: v1.0 · 2026-09-18（起草；brainstorm 期一次 overall 回填 v1.35 已并入，见 Section 3）
- **Status**: Draft
- **Author**: [human] · Claude Opus 5 (1M context) (osuperpowers:brainstorming)
- **Parent program**: [2026-09-13-osuperpowers-overhaul-overall.md v1.35](./2026-09-13-osuperpowers-overhaul-overall.md)（P6 行 scope/acceptance 已含全部 brainstorm 收敛 + v1.28–v1.35 全部登记）
...
│ 平面（plane）          │ 语言面        │ 缓存/契约面            │
├─────────────────────────────────────────────────────────────┤
│ ① osuperpowers 发布面   │ .mjs 单文件禁 │ report-templates 零依赖 │
...
> B6 需在删除一切消费面后进行（先删机制引用再拆 submodule，避免孤儿引用）。`markets in dev`：`git submodule deinit -f --all` 不会影响 .gitmodules 内容——需手动删 `.gitmodules` 三条 + `git add .gitmodules` + 删目录。
...
**E4 · 测试运行内存守卫**：reported 于域 C6（复核固化，不单独成 task——配置已落地，复核 + 断言）。
