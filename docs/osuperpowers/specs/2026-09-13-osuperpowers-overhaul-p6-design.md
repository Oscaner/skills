...
- **Version**: v1.12 · 2026-09-20（v1.11 之上回填 **执行层 WIP 保全合同**——用户 2026-09-20 编排实证（T23 implement attempt-1 exit 1 · branch-fix exit 143 SIGTERM scope 漂移 15 文件）授权；新增 spec T7.3 → plan Task 24；overall v1.50 同窗。v1.11 之上：v1.11 为 lifecycle 状态正交化（T7.2 → plan Task 23；overall v1.48 同窗）；v1.10 为 plan-constraints 物料化（T7.1 → plan Task 22；overall v1.47 同窗）
- **Status**: Draft
- **Author**: [human] · Claude Opus 5 (1M context) (osuperpowers:brainstorming)
- **Parent program**: [2026-09-13-osuperpowers-overhaul-overall.md v1.50](./2026-09-13-osuperpowers-overhaul-overall.md)（P6 行 scope/acceptance 已含全部 brainstorm 收敛 + v1.28–v1.50 全部登记）
...
| T7.3 | E2/C-3 | **执行层 WIP 保全合同（v1.12，用户 2026-09-20 编排实证）**——两次大型派发中途死亡（T23 attempt-1 exit 1 · branch-fix exit 143 SIGTERM 15 文件 scope 漂移）暴露：**① WIP 保全**——非 exit-0 + 树脏派发 engine 自动 `git stash push -u`（纯对象库零 ref 污染）+ workspace 记 stash ref + 规模（文件数/+M/−M）进 BLOCKED blocker/notes——执行失败产物保全可评审可抢救（Flow Atomicity「提交=评审产物」不变，非提交不再=必然销毁）；**② findings-scope lock**——fix/review 注入 findings 声明文件集 + finalize diff 越界 BLOCK（T11 pending-acceptance 的代码镜像；review findings 附带文件集元数据）；**③ 死亡诊断**——EXECUTION_FAILURE blocker 扩为 exit code + WIP 快照 ref + 改动规模 + 死因线索（agent 报错 exit 1 vs 外部信号 143 vs TIMEOUT 可区分重放）| dispatch/base.ts · dispatch/task.ts · cli/branch-fix.ts · rules/failure.ts · review findings schema（文件集字段）· workspace artifacts · tests |
