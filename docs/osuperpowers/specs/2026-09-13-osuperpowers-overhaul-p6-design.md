...
| T7.8 | E2/D-1 | **相位状态列状态机 + 回填义务（v1.19，P6 closeout 高维复盘）**——closeout 暴露：`[Pending]` 无义务死信（P6 行全程 pending、T14 行无向上传导、claim 校验只查终态）。统一抽象 = 相位状态列 = 状态机（Pending → doc-version → Done），转换三执法位：**① validate 机械守卫**（overall-consistency，maintainer 侧）——行形守卫（同文件 rows cell 数一致，结构错位即 FAIL）+ **状态滞留守卫**（phase 的 spec 文档 ∈ 树 ⇒ Design 列不得 [Pending] 且须含自身 `<Pn>-design` token；plan 文档 ∈ 树 ⇒ Plan 列不得 [Pending]——文档落地即强制回填，否则 CI FAIL）+ **closeout-claim 守卫**（Plan 列 Done 声明 ⇒ 读引擎 `.osuperpowers/cdd/<slug>/progress.json` 校该 plan 全 task complete；workspace 缺失跳过＝dev 侧实证，同 cache 诚实边界）· **② skills 边界义务**——writing-phase-spec `commit-spec` 后同步 parent overall Design 列 = 新 spec 版本（changelog 同窗）/ writing-plans `commit-plan` 后同步 Plan 列 = approved-marker / cli-driven-development·finishing 终态 Done 回填保留 + closeout claim 匹配 progress / writing-overall-spec 登记流程写义务（记录在案，流程即执法位）· **③ 引擎**零新写入面（overall 为编排面；progress.json 已存在 = closeout-claim 消费的任务事实源；T14 类遗留 = 终态声明与任务事实绑定被机械抓住）。验收主线：`[Pending]` 从无义务死信变有义务状态（任何文档落地/任务完成当下状态列必同步、机械可测） | scripts/validate/overall-consistency.ts + fixtures/tests · skills/{writing-phase-spec,writing-plans,writing-overall-spec,cli-driven-development,finishing}/SKILL.md · .osuperpowers/cdd/<slug>/progress.json（读侧） |
