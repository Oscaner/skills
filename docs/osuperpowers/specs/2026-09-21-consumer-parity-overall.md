...
7. closeout 统一规则：声明源 ↔ 列双向全列（含 engine 派生终态并入声明源）；**二门面判读（v1.11 spec-review r2 回填）**——**结构性面**（缺失 cell / 缺 claim）：pre-flight mismatch 非空 → BLOCK + 指引；**终态欠账面**（plan-complete 未回填）：pre-flight fail-open（不 BLOCK）、post-flight plan-complete 且未回填 → 高亮回填 recommand（回填由 orchestration 执行，2026-09-21 grilling 裁决）；**lane 边界**：branch 通道 pre-flight 排除终态欠账成员（其闭环 dispatch 恒位于 finishing 回填之前、不可能成为合法咬合点——时序语义推导，非 lane 豁免常量），硬门只压结构性面
