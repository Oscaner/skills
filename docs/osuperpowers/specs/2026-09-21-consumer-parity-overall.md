...
| P3 | **本仓校验面重建**（2026-09-22 grilling 合流，v1.14 修正三处）：`scripts/validate/overall-consistency.ts` + `plan-spec-anchors.ts` **退役**（42 用例 **全删零迁入**——判据面 engine 测试已用 mkdtemp 自造链覆盖；P3 design 落 42 行逐案对照表：repo 断言 → engine 对应用例:行 → 覆盖判定）；**接线重构为零接线**——四表判据面唯一实现 = engine lifecycle（docContractValidate），本仓运行期每步 dispatch 自身被审（canary 实证回归运行期形态）、测试面 = engine 5b1 套件自造链；**`canary-dogfood.test.ts` 删除**（产物耦合病：硬编码本仓真实文档路径为 fixture，产物删即测试废——unit/e2e 测试 = 功能性验证，不允许以本仓产物为 fixture，2026-09-22 用户裁决）；validate **无四表 block**（steps 12 → 11，ci-validate.test.mjs / pre-commit.test.ts 钉死值同步）；**steps 全语义名零编号 anchor**（`5b0/5b1/5b/12. overall consistency` 族退役——validate 接线重构时整名语义化，无意义 anchor 不利运维，2026-09-22 用户裁决）；**canonical phase-id 语法严格 A**（`^P\d+(\.\d+)*$`——点分数字分层，弃用字母后缀 `P2a`/`P2.a`/`P2.1` 等变体不兼容现有体系 → 统一点分级；canonical schema 四 pattern（phaseId · idFormat · designToken · range）单点派生 + **schema description 写明语法与分层语义**，2026-09-22 用户裁决）；S3 活文档残留簇改写（overall 四条 · osuperpowers-plugin.md:116-120 · CLAUDE.md:47 · overall-spec SKILL「block 12 dogfood」句 → 守卫退役后的 engine 执法位表述）；F8a `\bH1\b` 盲区随守卫文件删除自消（不扩 ALL_MECH_POSITIONS）；`smoke-cdd` 升级 consumer-sim（pnpm pack → 临时仓安装 → 消费者视角跑引擎链）挂 release 门；**运维规范落档**（零产物 fixture · 零编号 anchor · 零 legacy 豁免死代码 · phase-id 语法 A —— 2026-09-22 grilling 裁决同步进 docs/maintainers） | [Pending] | [Pending] | `scripts/validate/` 无 charter 守卫残留（grep/sweep 实证 + 42 行逐案对照表，grep 零残留）；case 删面 = 零功能损失（逐案对照 + engine 测试面覆盖实证）；validate 无四表 block、steps 全语义名零编号 anchor（`\b(?:5b\d?\|12\.)\b` 族零残留）；`canary-dogfood.test.ts` 删除后 engine 套件零本仓产物 fixture 引用（grep 实证）；canonical phase-id 语法 A 落地（四个 schema pattern 单点派生 + description 全写 + engine 消费面零误伤、split-phase 测试改造）；本仓程序文档链经 P3 全程 dispatch 审计零失败（canary 运行期实证）；consumer-sim 黑盒在发布门可跑并输出消费者等效结果；`pnpm run validate` 重构后全绿（11 块）；运维规范落档（docs/maintainers，2026-09-22 裁决六项） | P2 ->(hard) |
