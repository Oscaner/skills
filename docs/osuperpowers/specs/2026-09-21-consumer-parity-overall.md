...
| v1.17 | 2026-09-22 | **P3 design spec-review-3 回填（spec-review-4 修正探针）**：P3 acceptance 零编号 anchor 探针改前缀锚定/整串判定（`(?:^\| )\b(?:[0-9]+\.\|5b\d*\|5c\|8-10)[. ]+[A-Za-z(]` 数字前缀步名全捕获、语义名零误报——node 断言「现名全命中 / 语义名全 miss」防白绿；旧式 `[0-9]+[. ]`+尾 `\b` 失效：尾 `\b` 在「.」→「 」non-word→non-word 边界断、吞点后备选项失配，`12. x`/`7. x`/`6. x`/`0. x` 全部漏检，spec-review-4 实证后改正向后缀 `[A-Za-z(]`，design AC4 同步）；consumer-sim pack 机制实证澄清（`prepare: dev:stub` 钩子移除为主修 + `pnpm pack --config.ignore-scripts=true` 备防；`--ignore-scripts` 旗标 / `npm_config_ignore_scripts` env / root 相对路径 pack 均实证失效；发布路径 `pnpm publish` 同险登记）——细节落 P3 design §2.6/AC6（v1.16→v1.17） | [human] · Claude Opus 5 (1M context) |
