...
| v1.17 | 2026-09-22 | **P3 design spec-review-3 回填**：P3 acceptance 零编号 anchor 探针改前缀锚定/整串判定（`(?:^\| )\b(?:[0-9]+[. ]\|5b\d?\|5c\|8-10)\b` 全残留捕获——dot 后尾 `\b` 盲点消除、`12. x`/`7. x`/`0. x` 数字前缀步名不再漏检，design AC4 同步）；consumer-sim pack 机制实证澄清（`prepare: dev:stub` 钩子移除为主修 + `pnpm pack --config.ignore-scripts=true` 备防；`--ignore-scripts` 旗标 / `npm_config_ignore_scripts` env / root 相对路径 pack 均实证失效；发布路径 `pnpm publish` 同险登记）——细节落 P3 design §2.6/AC6（v1.16→v1.17） | [human] · Claude Opus 5 (1M context) |
