...
- **C2 结构性**——断言对象是结构 token / heading / 模板 → 一律同源派生（d4t 单源），engine 运行时同构消费；repo 侧任何人肉第二副本 = 违规。
- **C3 退化（canary）**——本仓任何「仅本仓侧 charter 执法」依赖 → 处置二选一：并入 engine 或删除；useless 必删、不留历史叙述豁免（2026-09-21 用户裁决）。
...
| ↑ | :121 | J2 | 保留（Phase inventory P2 已完成阶段的范围叙述："overall-consistency validator glob + fixtures 迁移"） |
| ↑ | :289 | J2 | 保留（change-history v1.6 记录：P2 plan 列归一要求，历史叙述） |
| ↑ | :290 | J2 | 保留（change-history v1.7 记录：canonical 列归一，历史叙述） |
| ↑ | :321 | J1 | 改写为历史时态（③ 段"`overall-consistency` = 本程序 charter 四表守卫（maintainer-mode dogfood）"主张中性化；change-history 事实保留） |
| `2026-09-13-osuperpowers-overhaul-p2-design.md` | :50 | J2 | 保留（§2.3 节标题 "validator — `overall-consistency` 单根收敛"，迁移设计档案） |
| ↑ | :52 · :56 | J3 | 保留（文件操作记录：`overall-consistency.mjs:32,39,40,461` / fixtures 路径） |
| ↑ | :113 · :125 | J2 | 保留（当时验证预期 / AC6 验收记录） |
| `2026-09-13-osuperpowers-overhaul-p2.md` | :7 · :106 · :108 · :116 · :133 · :147 · :162 · :163 · :383（共 9 处——AC2 pattern 全量命中行） | J2/J3 | 全部保留为历史执行记录；逐处划类见 §2.3.1，**不改写** |
| `2026-09-13-osuperpowers-overhaul-p3-design.md` | :173 · :185 | J2 | 保留（当时验证预期 / AC7 验收记录） |
| `2026-09-13-osuperpowers-overhaul-p3.md` | :773 | J3 | 保留（changeset 文件名清单归档） |
| `2026-09-13-osuperpowers-overhaul-p4.md` | :191 | J3 | 保留（doc-root.mjs 修改指令中的文件引用） |
...
| ↑ | :166（Task 21 Do ④ 段） | J3 | 保留（测试就近 `__tests__` 迁移指令中的文件路径列举） |
| 零命中 5 件（p1-design · p1-plan · p4-design · p5-design · p5-plan） | — | — | 全量 grep 复查（committed tree 实测，见 Acceptance）确认零命中 |
...
| 行 | 内容概要 | 划类 | 理由 |
|---|---|---|---|
| 7 | Architecture 迁移两层（机制层含 `overall-consistency` SPECS_DIR/PLANS_DIR 单根） | J2 | 当时架构规划叙述（历史时态） |
| 106 | Modify: `scripts/validate/overall-consistency.mjs:32,39,40,461` | J3 | 当时文件操作记录（旧 .mjs 名，已 .ts），归档 |
| 108 | Test: `node scripts/run.mjs validate`（block 12）+ `vitest run scripts/validate/overall-consistency.test.mjs` | J3 | 验证命令记录 |
| 116 | `scripts/validate/overall-consistency.mjs` 代码块首行 | J3 | 文件路径小节头（归档） |
| 133 | 建议的 change-history v1.7 行文案（含守卫字样） | J2 | 已入 overhaul overall 的 change-history（append-only 记录），本行是其写入指令副本 |
| 147 | `vitest run scripts/validate/overall-consistency.test.mjs`（fixtures 注入 roots） | J3 | 验证命令记录 |
| 162 | `git add scripts/validate/overall-consistency.mjs …` | J3 | 提交文件清单归档 |
| 163 | `git commit -m "refactor(validate): overall-consistency 单根 …（v1.7）"` | J3 | 提交说明归档 |
| 383 | P2 docs 根落点迁移摘要（overall-consistency 单根收敛） | J2 | 当时任务范围摘要（历史时态） |
...
| 改动 | 文件 | 判据 | phase |
|---|---|---|---|
| 13 件 J1 改写（4 处） | overhaul overall:106/321 · p6-design:263 · p6-plan:134 | 裁决 A（useless 必删；overall:321 ③ 段与 :106 同主张——一并中性化） | P1 |
| J2/J3 保留登记（26 命中行/8 文件全量 grep 面） | p2-plan 9 处（§2.3.1 明细）· overall:121/289/290 · p2-design:50/52/56/113/125 · p3-design:173/185 · p3-plan:773 · p4-plan:191 · p6-plan:166 | 历史档案（不伪造历史） | P1（登记） |
