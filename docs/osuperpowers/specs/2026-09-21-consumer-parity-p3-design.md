...
|---|---|---|---|
| 1 | canonical 头 → canonical=true + phases 解析（id/design/plan 三列） | D:266（audit-clean 链：canonical 头 + P1/P2 phases 全解析零失败）+ D:633（非 canonical 头 failure 逆面）+ K:510-518（header/列解析） | 判据面已由 engine 测试覆盖 → 删 |
| 2 | 行形守卫：同文件 phase 行 cell 数不一致 → phaseShapeErrors + checker throw（shape-drift） | D:650（overall row-shape drift → failure）+ K:534-537（shapeDrift 收集、错位行跳过） | 判据面已由 engine 测试覆盖 → 删 |
| 3 | slug 由文件名剥离日期前缀 + -overall 后缀 | K:715-718 fileNameSlug（剥离日期前缀 + `-overall` 后缀，同语义）+ D:266（fixture `2026-09-21-plan-*.md` 日期前缀 slug 实测） | 判据面已由 engine 测试覆盖 → 删 |
| 4 | Version: vX.Y + 升序（v1.0→v1.1→v1.2）→ 通过 | D:266（AUDIT_OVERALL 三段升序零失败）+ K:596-599（ascending 规则） | 判据面已由 engine 测试覆盖 → 删 |
| 5 | 升序破坏（v1.1 后 v1.0）→ throw | D:673（change-history not ascending → failure）+ K:596-599 | 判据面已由 engine 测试覆盖 → 删 |
| 6 | 版本重复（两行 v1.1）→ throw（③ 重复检测） | K:594（duplicate version 收集）+ K:680-688（versionProblems 同 field 上抛）+ D:673（同一「Change history」field 钉死；fix = strictly ascending unique, K:686） | 判据面已由 engine 测试覆盖 → 删 |
...
| 8 | 非 canonical 头 → canonical=false（不 throw） | D:633 + LV:150（非 canonical 头 → 结构化 failure，非 crash） | 判据面已由 engine 测试覆盖 → 删 |
| 9 | graph 引用 P9（inventory 无）→ throw（④b） | D:503（face ③ graph 悬垂 P9 → failure）+ K:943-953（graph token 成员判定） | 判据面已由 engine 测试覆盖 → 删 |
| 10 | dependency 列传 P1 前驱 → 通过（④b） | D:511（④逆面：dependency cell 前驱 P9 → failure）+ D:266（`P1 ->(hard)` 前驱 ∈ ids 零失败）+ K:954-966 | 判据面已由 engine 测试覆盖 → 删 |
| 11 | issue ref 畸形（#246#issuecomment-abc）→ throw（④c） | K:892-902（`#issuecomment-` 后非数字 → malformed）+ D:623（`#12x` → issue field failure） | 判据面已由 engine 测试覆盖 → 删 |
| 12 | ④c 宽松：`#246（session master body）` + `none` 行放行 | K:891（`none` 跳过）+ K:903（bare `#NNN` 后缀非标识符 → 放行，`（session master body）` 属此后缀）+ D:266（none 行零失败） | 判据面已由 engine 测试覆盖 → 删 |
| 13 | issue ref Phase 列 ∈ phaseIds → 通过（④c） | K:879-888（issue row phase ∈ ids 判定）+ D:615（④逆面 P9 → failure）+ D:266（P1 ∈ ids 零失败） | 判据面已由 engine 测试覆盖 → 删 |
| 14 | ①正向：plan-claim 列 [Pending] → throw | D:271（face ① forward：claim Done vs 列 [Pending] → failure）+ K:806-816（plan-link claim 提取） | 判据面已由 engine 测试覆盖 → 删 |
| 15 | ①括号可选：`Implementation plan Pending → Done` 实为 claim → 列 Done 通过（无 claim 报错） | K:764（`(?:Pending\|\[Pending\])` 括号可选显式支持）+ D:271（claim→列比较机制）+ D:266（claim 形态零失败） | 判据面已由 engine 测试覆盖 → 删 |
| 16 | ①区间展开：P1–P4/P6 声明 → P1..P4+P6 全列 Done 断言 | K:774-796 phaseIdsIn（RANGE_RE 区间端点展开 P1–P4 → P1..P4）+ D:271/394（claim 解析机制） | 判据面已由 engine 测试覆盖 → 删 |
| 17 | ①单行混合（cdd v1.27 形态）：同一行 plan 区间 + design 声明 → 全断言 | K:803-829 extractClaimRows（单 summary 分句遍历 plan + design 双 link word）+ D:271/292（plan/design claim 面） | 判据面已由 engine 测试覆盖 → 删 |
| 18 | ①CLAIM_RE 尾随标点防御（warn fix 回归）：`Pending → Done。` / `→ Done——` 目标不含标点 | SCH `claimPatterns.claimClause.pattern`（target 停集含 `。`/`——` 等 CJK 标点）+ K:754-758 claimKey（尾随闭合括号剥离）+ D:304（claim 解析成功面） | 判据面已由 engine 测试覆盖 → 删 |
| 19 | ①design-claim 列 [Pending] → throw（指定 §2.5 item 8） | D:292（face ① design：claim 钉 token 列未带 → failure）+ K:817-825（design-link claim 提取） | 判据面已由 engine 测试覆盖 → 删 |
| 20 | ①反向：plan 列 Done 但全史无 plan-claim → throw | D:281（face ① reverse：plan 列 Done 无对应 claim → failure）+ K:806-816 | 判据面已由 engine 测试覆盖 → 删 |
| 21 | ②文档存在：slug glob 命中跨日期文档（overall 2026-09-05 → p2 2026-09-07）→ 通过 | D:496（②逆面：non-pending plan 列无 doc → failure）+ D:266（glob 命中跨日期文档零失败）+ K:973-976（plan 双形 glob） | 判据面已由 engine 测试覆盖 → 删 |
| 22 | ②design 跨引用忽略：P2 列含 `（源 P3-design）` 只断言 p2 design 文档 | K:739-744 ownDesignToken（`（源 P3-design）` 跨引用非 own token）+ D:266（own token 断言零失败）+ D:489（②逆面：own token 缺 doc → failure） | 判据面已由 engine 测试覆盖 → 删 |
| 23 | ④a：phase 文档锚点 #999#issuecomment-… 不在注册域 → throw | D:600（face ⑤：phase doc 锚点不在 Issue inventory → failure）+ K:917-939（anchor registry 扫描） | 判据面已由 engine 测试覆盖 → 删 |
| 24 | ②plan 双形：无后缀 plan 命中（既有 shipped 路径 span-mixed 回归）→ 通过 | K:975 plansHit（bare 形 glob）+ D:496/266（②面契约：plan doc 存在性） | 判据面已由 engine 测试覆盖 → 删 |
| 25 | ②plan 双形：`-plan.md` 变体命中（planvar p2 仅 -plan 形）→ 通过 | K:975 plansHit（`-plan` 变体形 glob）+ D:496（②面契约：plan doc 存在性） | 判据面已由 engine 测试覆盖 → 删 |
...
| 31 | clean：../specs/ 相对形 + v1.1/v1.2 行迹 → 零漂移 | D:115（validSpec 相对形 parent + lineage v1.0 匹配零失败）+ D:704（pinned token ∈ lineage → 零失败）+ CO:122（overallPath 暴露 Class B lineage） | 判据面已由 engine 测试覆盖 → 删 |
| 32 | 目标文档不存在 → parent-unresolved | D:185（Parent program target unresolvable → chain truncation） | 判据面已由 engine 测试覆盖 → 删 |
| 33 | 目标存在但非 -overall.md → parent-notoverall | D:190（Parent program 非 `*-overall.md` → chain truncation）+ K:624（`-overall.md` 判定） | 判据面已由 engine 测试覆盖 → 删 |
| 34 | 行内 vX.Y ∉ 目标 overall 版本行迹（header ∪ Change history）→ parent-version | D:696（spec 钉 v9.9 → OVERALL face 失败，单一实现）+ K:689-705（merged version-lineage） | 判据面已由 engine 测试覆盖 → 删 |
...
| 37 | drift → 抛 ANCHOR DRIFT（含 kind/file:line/target） | D:129/138（failure 带 artifact/file/field/missing/fix guidance）+ D:752（failure line 形状钉死） | 判据面已由 engine 测试覆盖 → 删 |
...
| 40 | isLegacyRef — 已删/迁面（_docs/ · pre-P2 docs/superpowers/ · controller-handoff.md）豁免 | K:833-845（审扫面仅 `-${slug}-p\d+` 阶段文档——pre-P2/`_docs/` 迁面路径不落审扫面，消费者环境无此存量）+ K:103-111（占位/机制非锚判定类）+ D:159（机制豁免用例） | 判据面已由 engine 测试覆盖 → 删 |
