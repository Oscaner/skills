...
- **Parent program**: [consumer-parity overall v1.24](2026-09-21-consumer-parity-overall.md)
...
Cross-phase 规则以 parent overall v1.24 为准（overall wins on conflict）：
...
- **重新整理**（2026-09-23 用户裁决修正 v1.20「内容保真迁移」口径——PR #275 评审回馈：运维文档没有做整理，要求是**重新整理重新编号，包括精简和删除**）：逐文件裁除与 CLAUDE.md / 各包 README / 他文重复的散文、删除死机制与退役陈述（flow-spec 零散文原则延续：不重述 skill/engine 流程规范，仅点指向）；明确同类项可合并成篇；**删除 `07-osuperpowers-plugin.md`**（用户裁决「没什么必要可以删除」——其唯一值面 repo release 流程已由 `.changeset/README.md` §Release flow 全量承载；live 引用清扫见 plan Task 9）；删除/合并后编号重排为连续 `NN-name.md`；互链同步更新、零断裂
...
- **英文 README 零中文（用户裁决 2026-09-23）**：`**/README.md`（根 + 两包三件 English）语言切换行 `[English](README.md) | [中文](README.zh-CN.md)` 的 `[中文]` 标签为唯一 CJK 违规面——改为纯英文标签 `[English](README.md) | [Simplified Chinese](README.zh-CN.md)`（三件一致，链接目标不变；`.zh-CN.md` mirror 保留 `[中文]`，中文主源非违规）
...
- 根 + 两包 `README.md` 零 CJK（node 断言 `[一-鿿]` 零命中）；切换行 `[Simplified Chinese]` 三件一致（grep 3 命中）
...
| 英文 README 语言切换行沿用「中文」标签 | `**/README.md` 零 CJK——切换行 `[中文]`→`[Simplified Chinese]`（根 + 两包三件一致；`.zh-CN.md` mirror 不动） | Yes — v1.24 · 2026-09-23 |
