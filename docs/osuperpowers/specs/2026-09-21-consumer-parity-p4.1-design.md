...
- 语言政策：程序文档中文主源（Strategy B）；maintainers / README / CLAUDE.md 英文主源（Strategy A / B extension），**mirror 仅 README.zh-CN.md**（政策修订随本 phase 落笔）
- 本 phase 零 engine 变更、零 scripts/validate 变更——纯文档面；改动限 `docs/maintainers/`、根 `README.md`、`README.zh-CN.md`、`CLAUDE.md`，均非 emit 派生输入（aff5c809 实证：docs/maintainers 修改后 precommit emit freshness 全绿）

## Section 2: Design body

**工作分解：三块 + 一个前置**——A. charter v1.20 基线确认（程内已落地，P4.1 落笔期不再改版本，按需四表保卫）；B. `docs/maintainers/` 重组 + 编号；C. `README.md` / `CLAUDE.md` 全面重写 + `README.zh-CN.md` 同步 mirror。
...
- 互链迁移：全族现 11 处相对 `.md` 链接（8 处集中在 README.md）、零 wiki 链接——rename 后全量更新、零断裂（断裂 = BLOCK）
...
- `README.md`（英文主源，弃现有章节分布）：目标骨架 9 段——定位段（一段话：多 harness AI 编码技能市场 + 一方/上游来源声明）→ 插件表（包 / 版本 / 来源）→ 安装（插件市场 · npm · 上游插件 · per-harness 对照）→ 快速开始 → 架构（packages 布局 + emit 派生链）→ 各包文档 → 开发（常用操作 · 新增一方插件 · 分支流程）→ License；`README.zh-CN.md` 为同步翻译 mirror，头部声明 mirror 关系 + 同步时间戳
- `CLAUDE.md`（弃现有章节分布）：目标骨架 7 段——仓库目的 → ⚠️ emit 产物是派生（置顶，repo 最常错警示）→ 关键命令表（emit / emit:check / validate / changeset / version）→ 架构（包即源 · cdd-engine 直调开发链路 · 语言政策 · Review Convergence）→ 提交与维护约定（Git 约定 · changeset 义务 · memory → docs/maintainers）
...
- 超出文档面的分歧（engine/产物行为与文字不符）→ finding/backfill 上抛（C3 判据），不在 P4.1 内改代码
...
- `README.md` / `CLAUDE.md` / `README.zh-CN.md` 按 §2 目标骨架重写落地（目标顶层章节标题逐条命中：grep 断言 9 段 / 7 段骨架）
- `README.zh-CN.md` 头部含 mirror 声明行（mirror 关系 + 同步时间戳）；全仓除 `README.zh-CN.md` 外零 `*.zh-CN.md` 文件（grep 断言）
...
- 重写后 README/CLAUDE.md 宣称面（harness 支持面 · 安装/来源 · 行为描述）与落地行为零分歧（逐条人工对照 + review 记档）
...
- 语言政策修订（mirror 仅 README.zh-CN）随 CLAUDE.md 重写落笔；需同步核 `docs/maintainers/skill-authoring.md` 语言主张与 `packages/osuperpowers/README.md` 中的引用口径
