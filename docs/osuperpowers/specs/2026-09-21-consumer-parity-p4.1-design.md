...
- **语言切换行保留 `[中文]`（用户裁决 2026-09-24 撤销 v1.24 零中文方向）**：三件 English `README.md`（根 + 两包）line 3 语言切换 `[English](README.md) | [中文](README.zh-CN.md)` 中 `[中文]` 为语言切换 UI 的合法原生标签，不构成「英文 README 混有中文」违规——**不走改动、维持原样**；v1.24 把其列为唯一 CJK 违规面并改 `[Simplified Chinese]` 的方向随裁决撤销回滚；`.zh-CN.md` mirror 同维原样
