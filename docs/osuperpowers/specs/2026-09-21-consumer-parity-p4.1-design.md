...
- 本 phase 零 engine 变更、零 scripts/validate 变更——纯文档面；改动限 `docs/maintainers/`、根 `README.md`、`README.zh-CN.md`、`CLAUDE.md`，均非 emit 派生输入（aff5c809 实证：docs/maintainers 修改后 precommit emit freshness 全绿）。**范围例外两条**：(1) parent overall（`2026-09-21-consumer-parity-overall.md`）的 backfill-overall / v-bump 更新——AC6 · 四表纪律的 branch-review 前前置义务；(2) `packages/osuperpowers/README.md` 的 maintainers 入链引用口径同步——rename 后零断裂所需，非包内容面变更、不属 emit 输入面
...
- `README.md` / `CLAUDE.md` / `README.zh-CN.md` 按 §2 目标骨架重写落地（目标顶层章节标题逐条命中：grep 断言 8 段 / 5 段骨架）
