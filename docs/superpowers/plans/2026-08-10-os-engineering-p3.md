# os-engineering P3 实施计划：薄封装 + 多 harness 发射
...
- 终态边界：overrides = 触发路由器（plugin-root，claude+cursor，**无技能体**）；os-engineering = 技能 + 引擎 + gate + 全 harness emit
...
| `plugins/os-engineering/tests/rule-reference.test.py` | Modify（仅语义模式） | T2 |
| `plugins/superpowers-overrides/tests/*`（validate-overrides-build / trigger-patterns） | Modify（spor-* 断言移除） | T1/T2 |
| `plugins/superpowers-overrides/bin/{cdd-orchestrator-gate.sh,cdd-session-activate.sh,override-claude-sdd-gate.sh,override-cursor-sdd-gate.sh}` | Move → os-engineering（adapters 改名 cdd） | T3 |
| `plugins/os-engineering/hooks/hooks.json` | Create（PreToolUse） | T3 |
| `plugins/os-engineering/skills/os-init/SKILL.md` | Create（参数化） | T4 |
...
```json
{
  ...
  "targets": [
    { "name": "os-engineering:os-brainstorming", "overrides": "superpowers:brainstorming", "source": "../os-engineering/skills/os-brainstorming" },
    { "name": "os-engineering:os-writing-plans", "overrides": "superpowers:writing-plans", "source": "../os-engineering/skills/os-writing-plans" },
    { "name": "os-engineering:cli-driven-development", "overrides": "superpowers:subagent-driven-development", "source": "../os-engineering/skills/cli-driven-development" },
    { "name": "os-engineering:os-executing-plans", "overrides": "superpowers:executing-plans", "source": "../os-engineering/skills/os-executing-plans" },
    { "name": "os-engineering:os-finishing", "overrides": "superpowers:finishing-a-development-branch", "source": "../os-engineering/skills/os-finishing" },
    { "name": "os-engineering:os-debugging", "overrides": "superpowers:systematic-debugging", "source": "../os-engineering/skills/os-debugging" },
    { "name": "mattpocock-skills:tdd", "overrides": "superpowers:test-driven-development", "source": null },
    { "name": "os-engineering:os-verification", "overrides": "superpowers:verification-before-completion", "source": "../os-engineering/skills/os-verification" },
    { "name": "os-engineering:os-code-review", "overrides": "superpowers:receiving-code-review", "source": "../os-engineering/skills/os-code-review" },
    { "name": "os-engineering:os-finishing", "overrides": "superpowers:using-git-worktrees", "source": "../os-engineering/skills/os-finishing" }
  ]
}
```

> 若 `source` 字段约束为 overrides 插件内路径（当前 schema 校验），改用 `target_plugin: "os-engineering"` + `target_skill: "os-brainstorming"` 等语义字段（修改 manifest schema + 生成器）；实现时以现有生成器读取逻辑为准扩展。

- [ ] **Step 1b: 放宽 manifest schema + manifest_targets.py（blocker）**

当前 `overrides-manifest.schema.json` 的 `name` 约束 `^spor-[a-z0-9-]+$`、`source` 约束 `^\./skills/`，且 `build/lib/manifest_targets.py` 有 `name.startswith("spor-")` 断言 —— 新目标表（`os-engineering:*` / `mattpocock-skills:tdd`、跨插件 source）会被硬拒。**同步修改**：
...
`build/generate-all.sh` / render 脚本的注入措辞从「MUST invoke superpowers-overrides:spor-<X>」改为「MUST invoke <目标>」（目标 = manifest name 字段，如 `os-engineering:os-brainstorming` / `mattpocock-skills:tdd`）。Cursor detect/enforce 指向新目标。

**删 `/spor-*` matcher 家族（blocker）**：`render-claude-hooks.sh` 的 `cc_matcher_spor_slash`（生成 `/spor-<slug>` UserPromptExpansion block）与 `render-hook.sh` 的 `/{t.name}` case（T1 后会对 os-engineering:os-brainstorming 生成 bogus 模式）—— **全部删除**。spec A2 的 `^/spor-*` matcher 同步移除（过渡别名结束）。触发模式只保留 `^superpowers:` + 裸 `/<upstream-slug>`。

**路由器不写 pending（blocker）**：`render-hook.sh` + `render-cursor-hooks.sh` 里生成 `sdd_activate` / `sdd_session_key` 块（调 `${_plugin_root}/bin/cdd-session-activate.sh` 写 pending）—— spec B1 规定「overrides 路由器不写 pending」。**删除这些调用**，`sdd_activate` / `sdd_session_key` 变量改名/移除（避免 T7 零残留 grep 命中 `sdd_`）。pending 写入归 os-engineering（gate 迁入后）。

> 注：T1 与 T3 都编辑 `build/render-cursor-hooks.sh`（T1 删 pending block，T3 删 preToolUse gate 条目）—— T3 是对 T1 已改文件的后续编辑，重跑 generate 后生效。

- [ ] **Step 3: 更新触发测试 + 路由器测试**

`trigger-patterns.test.py`：注入内容断言改为目标技能（os-engineering:* / mattpocock-skills:tdd），触发模式（`^superpowers:` + 裸 `/<slug>`）保持。

**另 3 个路由器测试同步 retarget**：`override-prompt-expansion.test.sh` / `override-cursor-detect.test.sh` / `override-cursor-enforce.test.sh` 硬断言 `spor-brainstorming` 名、`superpowers-overrides:spor-*` 引用、`$ROOT/skills/spor-*` 路径 —— 全部改为 os-engineering:* / mattpocock-skills:tdd + os-engineering 技能路径。

**manifest 目标存在性校验**：T1 加一个跨插件目标存在性检查（每个目标解析到 `plugins/os-engineering/skills/<name>`；mattpocock tdd → `plugins/mattpocock-skills/skills/engineering/tdd/SKILL.md`（**嵌套路径**，非 `skills/tdd`）；缺失 → 报错），挂入 validate-overrides-build.sh / ci-validate。
...
- Modify: `plugins/os-engineering/tests/rule-reference.test.py`（仅语义模式）
...
`plugins/os-engineering/tests/rule-reference.test.py`：删除数字模式分支（overrides 无技能可校验），入口只收 `os-engineering/skills:semantic`；`ALLOWLIST_NUM` 相关死条目清理。`validate-overrides-build.sh` / ci-validate 中 rule-reference 调用同步改单语义。
...
```bash
git add -A plugins/superpowers-overrides plugins/os-engineering/tests
git commit -m "refactor: delete spor-* skills, retire rule-reference numeric mode"
```

---

### Task 3: gate 迁入 os-engineering

**Files:**
- Move（改名）: `plugins/superpowers-overrides/bin/lib/cdd-orchestrator-gate.sh` → `plugins/os-engineering/bin/lib/cdd-orchestrator-gate.sh`
- Move（改名）: `plugins/superpowers-overrides/bin/cdd-session-activate.sh` → `plugins/os-engineering/bin/cdd-session-activate.sh`
- Move（改名）: `override-claude-sdd-gate.sh` / `override-cursor-sdd-gate.sh` → `plugins/os-engineering/bin/override-claude-cdd-gate.sh` / `override-cursor-cdd-gate.sh`
- Create: `plugins/os-engineering/hooks/hooks.json`（PreToolUse Write|Edit + Bash → cdd-orchestrator-gate）
- Create: `plugins/os-engineering/hooks/hooks-cursor.json`（cursor preToolUse → override-cursor-cdd-gate）
- Modify: `plugins/superpowers-overrides/hooks/hooks.json`（删 PreToolUse，只剩 UserPromptExpansion）
- Move: gate 测试（`sdd-gate-allow-deny-smoke.sh` / `override-*-gate.test.sh` / `sdd-gate-test-lib.sh`）→ os-engineering

**Interfaces:**
- Consumes: 无
- Produces: gate 在 os-engineering 生效；overrides 无 PreToolUse

- [ ] **Step 1: 迁移 + 改名 gate 脚本**

```bash
mkdir -p plugins/os-engineering/hooks
git mv plugins/superpowers-overrides/bin/lib/cdd-orchestrator-gate.sh plugins/os-engineering/bin/lib/cdd-orchestrator-gate.sh
git mv plugins/superpowers-overrides/bin/cdd-session-activate.sh plugins/os-engineering/bin/cdd-session-activate.sh
git mv plugins/superpowers-overrides/bin/override-claude-sdd-gate.sh plugins/os-engineering/bin/override-claude-cdd-gate.sh
git mv plugins/superpowers-overrides/bin/override-cursor-sdd-gate.sh plugins/os-engineering/bin/override-cursor-cdd-gate.sh
# 适配器内 lib source 路径同步（相对路径）
```

**生成器 hook 同步（blocker）**：`build/render-claude-hooks.sh` + `build/render-cursor-hooks.sh` 目前**生成** PreToolUse/preToolUse → override-*-sdd-gate.sh 条目 —— **从生成器删除这些 gate 条目**，重跑 `pnpm run generate:overrides` 重生成 overrides hooks.json（只余 UserPromptExpansion）与 hooks-cursor.json；`validate-overrides-build.sh` 的 PreToolUse 断言同步更新（gate hooks 归 os-engineering，不在 overrides 断言）。

- [ ] **Step 2: os-engineering hooks.json（PreToolUse）**
...
`git mv` gate 测试到 os-engineering（`sdd-gate-allow-deny-smoke.sh` / `sdd-gate-test-lib.sh` / `override-claude-sdd-gate.test.sh` → `override-claude-cdd-gate.test.sh` / `override-cursor-sdd-gate.test.sh` → `override-cursor-cdd-gate.test.sh`），引用路径更新；ci-validate / validate-overrides-build 的 gate 测试调用改 os-engineering。

**`sdd-orchestrator-line-budget.test.sh` 处置**：该测试引用已删的 spor-*（薄指针化后 body 断言失效）—— 迁移到 os-engineering 并重定向到 os-executing-plans + os-engineering docs（若仍有预算意图），或删除（其内容已被 rule-reference 双模式覆盖）。二选一，保持 validate 绿。
...
预期：gate 测试过（os-engineering 侧），overrides 路由器 hook 测试过，validate ALL PASS。

- [ ] **Step 6: 提交**

```bash
git add -A plugins/superpowers-overrides plugins/os-engineering scripts/ci-validate.sh
git commit -m "refactor: relocate orchestrator gate to os-engineering (PreToolUse hooks)"
```
...
**Files:**
- Create: `plugins/os-engineering/skills/os-init/SKILL.md`
- （spor-init 已在 T2 删除）
...
- [ ] **Step 1: 创建 `os-engineering/skills/os-init/SKILL.md`**
...
含版本戳 `<!-- os-engineering-version: <version> -->`（validate-overrides-build.sh 的 dogfood 断言读取；T5 版本化后同步）。自检表来源 = overrides manifest 目标表（单一 SOT）。

> **ci-validate 技能数断言**：os-init 加入后 os-engineering skills = **13**（12 发射 + os-init），ci-validate.sh 5b 的 EXPECTED 计数从 12 改为 13（或把 os-init 排除在计数外）。

- [ ] **Step 3: 提交**

```bash
git add plugins/os-engineering/skills/os-init
git commit -m "feat: add parameterized os-init skill (os-init spor)"
```

---

### Task 5: os-engineering 独立版本化

**Files:**
- Modify: `plugins/os-engineering/package.json`（版本接入 changeset）
- Modify: `scripts/version-packages.mjs` + `scripts/lib/version-utils.mjs`（双插件版本化）
- Modify: `marketplace/source.json`（os-engineering 版本同步）
- Modify（适配）: `.github/workflows/release.yml` / `ci.yml`

**Interfaces:**
- Consumes: 无
- Produces: os-engineering 独立 semver（0.1.x）经 changeset 发布

- [ ] **Step 1: version-packages.mjs 扩展双插件**

当前硬编码 `plugins/superpowers-overrides/package.json`。改为循环处理两个插件：superpowers-overrides（superpowers-相对 scheme）+ os-engineering（独立 semver）。`version-utils.mjs` 增加 os-engineering 的独立 semver 递增逻辑（minor/patch per changeset）。

- [ ] **Step 2: os-engineering package.json + source.json 版本**

`os-engineering/package.json` version 0.1.0（占位已有），接入 changeset 后 `pnpm changeset` 可递增。source.json 同步。**`.claude-plugin/plugin.json` 加 `version` 字段（0.1.0）作为 SOT**（os-init 自检表版本戳 + dogfood 断言读取；overrides 的 load_plugin_version 模式同理）。

- [ ] **Step 3: release 链适配**

`release.yml`：os-engineering **独立发版**（tag `os-engineering@0.1.x` + GitHub Release），与 superpowers-overrides 并行。单一路径，不做「与 overrides 同版本 PR」二选一。

- [ ] **Step 4: 提交**

```bash
git add -A scripts plugins/os-engineering/package.json marketplace/source.json .github
git commit -m "feat: independent versioning for os-engineering (changeset + release chain)"
```
...
```js
// 对每个 first-party 插件（superpowers-overrides, os-engineering）:
...
// 9. os-engineering: PreToolUse hooks
...
```bash
git add scripts plugins/os-engineering/.claude-plugin plugins/os-engineering/.cursor-plugin plugins/os-engineering/.codex-plugin plugins/os-engineering/.kimi-plugin plugins/os-engineering/gemini-extension.json plugins/os-engineering/GEMINI.md plugins/os-engineering/.agents plugins/os-engineering/package.json marketplace/source.json scripts/ci-validate.sh
...
- Modify: `plugins/os-engineering/skills/os-*/SKILL.md`（Read Upstream 解析适配 in-harness 优先）
...
- README 插件表：overrides = 触发路由器（无技能），os-engineering = 技能 + 引擎 + gate；删已删 spor-\* 行
- cross-harness-overrides.md：更新为「路由器（claude+cursor）+ os-engineering 多 harness 发射」模型，删 spor-\* 引用
...
对本仓库运行 `os-init spor`，把项目 CLAUDE.md / .cursor rules 的自检表更新为 os-\* 目标（当前表仍映射已删 spor-\*）；`validate-overrides-build.sh` 的 dogfood 断言版本戳改为 `os-engineering-version`。
