...
### Task 2: brainstorming/SKILL.md — 结构性重写（#162）
...
### Task 3: writing-plans/SKILL.md — 结构性重写（#156 / #163-① / #163-②）

**Files:**
- Rewrite: `packages/osuperpowers/skills/writing-plans/SKILL.md`

**Interfaces:**
- Consumes: 现有 writing-plans/SKILL.md（6 条 Rules，4 条 Red Flags）
- Produces: 统一骨架重写版本；Checklist（6 步）+ Plan Review HARD-GATE 嵌入 Rule: Plan Review via CLI + Rule: Section-by-Section 更新 + 9 条 Red Flags（引用路径更新为 docs-review.md）

- [ ] **Step 1: 读取现有文件**

  Read `packages/osuperpowers/skills/writing-plans/SKILL.md`，确认 6 条 Rules 名称和 4 条 Red Flags 内容。
...
- [ ] **Step 2: 写入重写版本**
  ...
  **Template resolution reuses** [Rule: Read Upstream](#rule-read-upstream) path rules (`{plugin-root}` = osuperpowers root). Dispatch discipline: see [docs-review.md](../docs/docs-review.md) (D1/D2/D3 + fresh-pass, mapped verbatim to cli). Stopping: Rule: Review Stopping in docs-review.md. Handoff output: Rule: Handoff Output in docs-review.md `[Engine pending P2]`.
...
- [ ] **Step 3: 自检**
  ...
  对照 spec Section 2.4 验证：
  - Checklist 6 条存在
  - Rule: Section-by-Section 含"写入粒度与确认时机解耦"说明，末行"禁止每节完成后暂停"（#156）
  - Rule: Plan Review via CLI 内部嵌入 HARD-GATE（#163-①），引用 `docs-review.md`
  - Red Flag "Plan Review 用内联自检替代 cdd-review CLI"存在（#163-①）
  - Red Flag "展示三选一选项"存在（#163-②）
  - Review Stopping × 2 条 Red Flag 存在
  - Execution Handoff 文本正确（非三选一）
  - 原有 4 条 Red Flags 保留（共 4 条原有 + 5 条新增 = 9 条）
...
- [ ] **Step 5: Commit**
  ...
  git commit -m "feat: rewrite writing-plans/SKILL.md with unified skeleton, HARD-GATE, and review stopping (#156 #163)"
  ```

---

### Task 4: executing-plans/SKILL.md — 结构性重写（#163-③ / #163-④）

**Files:**
- Rewrite: `packages/osuperpowers/skills/executing-plans/SKILL.md`

**Interfaces:**
- Consumes: 现有 executing-plans/SKILL.md（11 条 Rules，7 条 Red Flags）
- Produces: 统一骨架重写版本；顶层 HARD-GATE + Checklist（6 步）+ 11 条精简 Rules（含两处嵌入 HARD-GATE）+ 11 条 Red Flags。注意：Fix Loop 不引用 docs-review.md（task-review 维持现有 Fix Loop 机制）

- [ ] **Step 1: 读取现有文件**

  Read `packages/osuperpowers/skills/executing-plans/SKILL.md`，确认 11 条 Rules 名称和 7 条 Red Flags 内容（用于自检对照）。
...
- [ ] **Step 2: 写入重写版本**
  ...
  在此之前禁止任何 repo tool call。Do NOT accept a mode pre-selection from a prior skill's handoff — the orchestrator always asks directly.
  </HARD-GATE>
  ...
  **Final:** [osuperpowers:code-review](../code-review/SKILL.md) whole-branch in-session → clean → [osuperpowers:finishing](../finishing/SKILL.md).
  ...
  - "Use superpowers:subagent-driven-development / superpowers:executing-plans" → upstream subagent-driven-development references to superpowers:* must be explicitly mapped to osuperpowers counterparts
...
- [ ] **Step 3: 自检**
  ...
  对照 spec Section 2.5 验证：
  - 顶层 HARD-GATE（Mode Selection 启动约束）存在
  - Rule: Mode Selection 内部嵌入 CLI 禁止内联编辑 HARD-GATE（#163-③）
  - Rule: Per-Task Review 内部嵌入 Per-Task Review 门控 HARD-GATE（#163-④）
  - Rule: Fix Loop **不含** docs-review.md 引用（task-review 使用独立 Fix Loop 机制）
  - Checklist 6 条存在
  - 11 条 Rules 保留（Read Upstream / Mode Selection / Task Complexity / Confirm Once / Fix Loop / Confirm Seams / Per-Task Review / Quality Invariants / Orchestrator Checklist / D6 Aggregation / Ledger）
  - Red Flags：7 条原有 + 4 条新增（CLI 内联编辑 / Per-Task 跳过 / Per-Task 与 3-pass 混淆 / Fix Loop 与 docs-review 混淆）= 11 条
...
- [ ] **Step 5: Commit**
  ...
  git commit -m "feat: rewrite executing-plans/SKILL.md with unified skeleton and HARD-GATE (#163)"
  ```
