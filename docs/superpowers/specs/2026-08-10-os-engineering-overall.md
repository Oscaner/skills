...
| P3 | **薄封装 + 多 harness 发射**。superpowers-overrides 收缩为**触发路由器**（plugin-root，claude+cursor）：manifest 触发→目标表（spor-\* → os-\*/cli-\*/mattpocock tdd），hooks/expansion/自检表指向 os-\*/cli-\*，**spor-\* 全部删除**，rule-reference 数字模式退役。os-engineering = 技能 + 引擎 + gate：gate 全迁（cdd-orchestrator-gate + session-activate + adapters + PreToolUse hooks）、`os-init` 落位（参数化 `os-init spor`）、独立版本化（semver + changeset + version-packages 扩展）、**impeccable 模式 build.js 多 harness 发射**（12 技能 → 14 非 claude harness 目录，`.agents/skills/` 一等目标 + 上游 superpowers 整目录连带 + per-harness self-check/README）、去掉 cursor-plugins wrapper emit。README / cross-harness-overrides 文档更新。 | [Pending] | [Pending] |
| P4 | **原生 marketplace + 跨 harness gate adapters**。Gemini（gemini-extension.json）/ Codex（.codex-plugin）/ Qoder（.qoder-plugin）/ Pi（pi package）/ Grok（[[marketplace.sources]]，可选）/ Trae（extension，格式待研）原生插件清单；Grok / Qoder / Codex / Gemini / Vibe / Kiro 的 gate PreToolUse/BeforeTool adapters（各需原生配置 + 信任仪式；Copilot 因 matcher 忽略延后）；非 claude+cursor 路由扩展（仅当产品需求出现）。 | [Pending] | [Pending] |

## §3 Dependency graph (ASCII)

```
P1（插件骨架 + cli-* 家族 + droid/pi + 选择）──▶ P2（os-* 家族）──▶ P3（薄封装 + 多 harness 发射）──▶ P4（原生 marketplace + 跨 harness gate）
```

- P1 → P2：插件存在、模式确立、harness 机制与 cli-driven-development 就位后，os-* 才能引用它们。
- P2 → P3：薄封装需要 os-* 目标全部存在才能映射。
- P3 → P4：原生清单与跨 harness gate adapters 建立在 P3 的 build.js 发射 + gate 基础设施之上。
