...
| P7d | **旧命名全量清理（破坏性更新）**。全仓库 `os-` 前缀 + `engineering` 旧名引用归零。`os-*`：`"os-init"` 通道分类名 → `"osuperpowers:init"`；`os-brainstorming` 等旧技能名 → `osuperpowers:brainstorming` 等；`os-engineering` → `osuperpowers`；`os-*` 描述性统称 → `osuperpowers:*`；历史文档旧路径/旧技能名同步。`engineering`：emit 函数名 `engineeringClaudeHooks`/`engineeringCursorHooks`/`engineeringHooksFor` → `osuperpowers*`；`category: "engineering"` → `"osuperpowers"`；`keywords` 中 `"engineering"` → `"osuperpowers"`；注释中 `engineering` 描述性文字 → `osuperpowers`；`cursor-plugins/engineering` stale check 清理。验收标准：`grep -rn 'os-\|engineering' ... \| grep -v 'osuperpowers\|oscaner'` 输出为空（仅保留 `"category"` / `"keywords"` 已改、`engineeringVersion` 字段名另议）。 | [Pending] | [Pending] | ⏳ 未启动 |
...
- v4.1 · 2026-08-18 · **P7c 完成 + P7d 新增**：P7c 实现完成（11 commits，全仓库 5 维度 grep 验证通过）；原 P7d（文档更新）已合并入 P7b/P7c；新增 **P7d 旧命名全量清理**（破坏性更新）—— `os-*` 旧前缀 + `engineering` 旧名引用归零：`"os-init"` → `"osuperpowers:init"`，`os-*` 旧技能名 → `osuperpowers:*`，`os-engineering` → `osuperpowers`，emit 函数名 `engineeringClaudeHooks` 等 → `osuperpowers*`，`category`/`keywords` 中 `engineering` → `osuperpowers`，历史文档旧路径同步。验收：grep 输出为空。
