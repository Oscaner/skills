...
| P7d | **旧命名全量清理（破坏性更新）**。全仓库 `os-` 前缀 + `engineering` + `oscaner-engineering` 旧名引用归零。`os-*`：`"os-init"` → `"osuperpowers:init"`；`os-brainstorming` 等 → `osuperpowers:brainstorming` 等；`os-engineering` → `osuperpowers`；`os-*` 描述性 → `osuperpowers:*`。`engineering`：emit 函数名 `engineeringClaudeHooks` 等 → `osuperpowers*`；`category`/`keywords` → `"osuperpowers"`；注释/描述 → `osuperpowers`；stale path check 清理。`oscaner-engineering`：temp 目录名 `oscaner-engineering/pending-cdd` → `osuperpowers/pending-cdd`（`cdd-gate-core.mjs` + `cdd-session-activate.mjs` + 测试断言 + `cross-harness-overrides.md`）。历史文档旧路径/旧技能名同步。验收标准：`grep -rn 'os-\|engineering\|oscaner-engineering' ... \| grep -v 'osuperpowers\|oscaner'` 输出为空（`engineeringVersion` 字段名另议）。 | [Pending] | [Pending] | ⏳ 未启动 |
