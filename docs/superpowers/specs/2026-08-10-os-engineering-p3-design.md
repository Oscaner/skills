...
（os-engineering 的薄 manifest / GEMINI.md / .agents/skills 由统一 emit 工具生成。）
...
- **os-engineering 薄 manifest**（各指向 `./skills/`）：
...
**去掉 cursor-plugins wrapper emit**：source.json 删 os-engineering 的 `cursor: {displayName, skills}` 配置（并入统一 emit 的 cursor manifest）。
...
|---|---|---|
| P3 为「薄封装」仅 hooks/生成器/自检表重写 | 扩为「薄封装 + 统一 emit」：os-engineering 的 per-harness 发射改为 **superpowers 模式**（薄 manifest 指向 skills/，非副本），统一 emit 工具从 source.json 生成 first-party 全部产物 | 是（v1.9 P3 行） |
