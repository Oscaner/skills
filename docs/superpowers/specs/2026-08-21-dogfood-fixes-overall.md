...
| P5 | 三技能收敛 + branch-review CLI | 删除 `executing-plans`/`code-review`/`cli-code-review` 三技能（及 zh-CN 镜像），review 链路单一收敛到 `cdd-review.mjs --template {task-review\|branch-review}`；`cli-driven-development/SKILL.md` 新增 Rule: Final Review（末尾 branch-review CLI 收尾，基线 `origin/develop`）；`writing-plans` 交棒改指 `cli-driven-development`；获全局破坏性授权（B） | [Pending] `specs/2026-08-21-dogfood-fixes-p5-design.md` | [Pending] `plans/2026-08-21-dogfood-fixes-p5.md` |
