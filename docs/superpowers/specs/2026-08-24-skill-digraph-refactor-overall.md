...
| P11 | cli-research：以 `vendors/mattpocock-skills/skills/engineering/research/SKILL.md` 为基线，扩展 `cdd-task.mjs` 加 `--mode research`（复用 workspace/brief/handoff 基础设施；research 不写 commit、不走 commit-gate、无 task-review/fix-loop）；research 输出落盘 `docs/superpowers/research/YYYY-MM-DD-<topic>.md`（brief 写研究问题，handoff 含 `artifacts: { findings: "<path>" }`）；新建 `cli-research` skill（节点锚定式，调用 engine research mode）；更新 `osuperpowers:brainstorming` 的 `explore-context` 节点——用 `cdd-task.mjs --mode research` CLI 调用替代 Agent tool 手动 spawn（front load 确认后、background execution） | Pending | Pending | ① `cdd-task.mjs` 新增 `--mode research`（VALID_MODES 扩展）+ research 专用 workspace/brief 解析；② research 模式不触发 commit-gate（skip commit contract）；③ handoff 契约：research 模式 `{ status, artifacts: { findings } }`，无 commits 段；④ `docs/superpowers/research/` 落盘路径 + naming 约定；⑤ 新建 `cli-research` skill（节点锚定式）；⑥ brainstorming `explore-context` 节点改为 CLI 调用（替换 Agent tool）；⑦ zh-CN 同步；⑧ emit + validate 绿；⑨ CDD execution: workspace + handoff + ledger + Final Review | P3 -> P11 |
| P12 | cli-timeout：harness 终止长任务 CLI（如 Claude Code session 超时 kill `claude -p`）。**架构级方案**（推迟到本 phase 统一设计）：① 调研各 harness background 能力（Claude Code `run_in_background` / Cursor-agent / Droid / Pi 无文档化 background）；② `runner.mjs` spawnCapture 加 timeout（per-mode 默认值 + env 覆盖）；③ partial handoff（timeout 时写 partial status + 已完成部分 artifacts）；④ orchestrator retry 路径（timeout → 读 partial → 重 dispatch 或降级）；⑤ cli-research 也受 timeout 约束（research mode 同样有 partial handoff + retry） | Pending | Pending | ① 各 harness background 能力调研文档（harness-registry.json 扩展 background 字段）；② spawnCapture timeout 实现 + env 覆盖；③ partial handoff 契约（timeout 时写 partial）；④ orchestrator timeout 恢复路径入图；⑤ research mode 同样受 timeout 约束；⑥ zh-CN 同步；⑦ emit + validate 绿；⑧ CDD execution: workspace + handoff + ledger + Final Review | P3 -> P12 |
...
| P14 | brainstorming 流程调优：将「新增 phase 流程规范」从 overall spec 维护者约定（v1.19b Boundary rules）**固化进 skill 本身**——消费者在 brainstorming 中补新 phase 时不再犯「先 grilling 后落 overall / issue inventory 未同步」的错误。范围：① 扩展 `osuperpowers:brainstorming` 的 `explore-context` 节点——在「理解需求 / 拆分 phase」子步骤后新增 **pre-design 决策门**：若识别出新 phase（用户列出新 issue 集合 / 新功能需求），**先调用 `rule: sync-overall`** 同步 overall spec（Issue inventory 立即登记 → Phase inventory 新增 → Dependency graph 补边 → version+bump+change-history），且四表同步校验；② 新增节点 `sync-overall`（Do：读 overall spec → 落 Issue inventory + Phase inventory + Dependency graph + change-history；Exit：overall 更新且四表一致 → 进入该 phase 的 per-phase brainstorm）；③ grilling 纪律强化——grilling 仅讨论**已落 overall 的 phase 设计细节**，禁止在 overall 未同步前展开新 phase 的具体设计；④ `add-phase` 子图（`explore-context` 检测到新 phase → `sync-overall` → 回到 `explore-context` 重新评估范围 → grilling）；⑤ 配套 `brainstorming/docs/add-phase-protocol.md` 参考文档（流程 + 四表同步清单 + 反例）。**依赖 P10/P11/P12 落定后实施**（P14 改的是 brainstorming SKILL.md，P11 已改其 explore-context 节点为 CLI 调用，需先稳定）；P13 终扫需确认 P14 新增的 add-phase 节点 / sync-overall 节点不残留旧格式。**⚠️ 实时反例（v1.19c 写入时即犯）**：写本行时 P10 仅完成 design spec（未走完 plan→dev→merge），本 session 已错误展开 P14 的 design spec 撰写 + 3-pass review——同时违反两条纪律：① 串行纪律（P10 未完成即并行 P14）；② 先 grilling 后落 overall（P14 design 在 P14 phase 行稳定前展开）。**这两条正是 P14 要消灭的反模式，故本 phase 的验收须额外钉死「写 spec 时不得跨 phase 并行」**，且 P14 自身 design 文本（已生成的 2026-08-28-skill-digraph-refactor-p14-design.md）须待 P10-P12 全部 ship 后再正式走完 review→user-approve→writing-plans，当前为非法超前产物，标记为 stale 待重建 | Pending | Pending | ① `osuperpowers:brainstorming` SKILL.md 含 `sync-overall` 节点 + `explore-context` pre-design 决策门 + `add-phase` 子图，且 digraph 可见「检测到新 phase → sync-overall → 回到 explore-context」回边；② grilling 纪律在节点 Fail 字段或 Invariant 声明（禁止 overall 未同步前展开新 phase 设计）；③ **串行纪律**：本 phase 的 design/plan/dev 须在所有前置 phase（P10/P11/P12）merge 后才启动，且任一时刻仅一个 phase 处于活跃 brainstorm→dev 循环（新增验收钉死，防止 v1.19c 重演）；④ `brainstorming/docs/add-phase-protocol.md` 产出（含四表同步清单 + 反例 + 本次 v1.19c 实时反例）；⑤ zh-CN 同步（brainstorming SKILL.zh-CN.md + add-phase-protocol.zh-CN.md）；⑥ `pnpm run emit && pnpm run validate` 绿；⑦ CDD execution: workspace + handoff + ledger + Final Review | P3 -> P14, P11 -> P14, P14 -> P13 |
