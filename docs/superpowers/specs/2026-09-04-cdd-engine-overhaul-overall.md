...
| P5 | [#232](https://github.com/Oscaner/skills/issues/232#issuecomment-5549287756) | Enhancement Q — cdd-gate 冗余且不生效 → 整体删除（gate core + 11 adapters + hooks + init/emit 联动 + cli-shared CDD_GATE 注入）；**P5 brainstorm 扩展（v1.23）**：+ cdd select / cli-select / harness-detect / skills-probe（双份）整组删除 + `--harness` 参数删除 + channel 收敛到 claude/cursor-agent + per-harness emit 产物（.codex/.qoder/.kimi/gemini）清理（统一抽象：宿主即目标） |
| P5 | [#232](https://github.com/Oscaner/skills/issues/232#issuecomment-5612106797) | Enhancement — progress.json 所有权收归 engine（engineRecoveryCount 移入 engine；orchestrator 只读不写；实测手写 tasks 结构错 → `tasks.find is not a function` dispatch 失败） |
...
| v1.23 | 2026-09-09 | P5 brainstorm 落地：**Gate 移除 + harness 层全面清理**（统一抽象「宿主即目标」）——原 Enh Q gate 删除扩展为：+ `cdd select`/`cli-select`/`harness-detect`/`skills-probe`（双份，生产死功能实证 `opts.probeSkills` 恒未接线）整组删除 + `--harness` 参数删除（engine 内部 `detectCurrentHarness` 判 host，空→BLOCK）+ channel 收敛到 claude/cursor-agent（registry 7→2 键）+ install-harness/`bin/init/` 删除 + init 收缩 marketplace 指引 + per-harness emit 产物清理 + gate 语汇 residue guard + review/fix `--doc`→`--spec`/`--plan` 参数统一；P5 scope/acceptance/dependency 更新（P2 vendored manifests 保留 submodule 不可改）；P5-design 批准 | [human] · Claude Opus 4.8 |
| v1.24 | 2026-09-10 | P5 plan-review 期追加 **D13（vendored 发布面 gemini 清净）**：收敛 claude/cursor-agent 后 `vendor-assembly.mjs` 给 mattpocock 补写的 gemini 装配为未验证通道 → 删 mattpocock gemini 分支 + `thinGeminiExtension`/`geminiMarkdown` 整删（superpowers 自带 gemini 产物保留，submodule 不可改）；P5 scope/acceptance 同步；P5-design +v1.1 | [human] · Claude Opus 4.8 |
| v1.25 | 2026-09-10 | P5 执行期追加 **D14（progress.json 所有权收归 engine）**：T1 dispatch 实测 orchestrator 手写 tasks 结构错（对象当数组）→ `progressData.tasks.find is not a function` dispatch 失败 + engineRecoveryCount 零 engine 写入点（仅 skill 文档要求 orchestrator 写）→ 上报 #232 comment 5612106797 → engineRecoveryCount 由 runner 写入 + cli-driven-development §engine-recovery/§timeout-decision 去 orchestrator 递增指令 + orchestrator 只读；P5 scope/acceptance 同步；P5-design +v1.2 | [human] · Claude Opus 4.8 |
| v1.26 | 2026-09-10 | **P5 closeout**：P5 全部执行完毕（T1–T9，含 D14 progress.json 所有权收归 engine + D13 vendored gemini 清净）；P5 Implementation plan Pending → **Done**；changeset `p5-cdd-engine-overhaul.md`（`@oscaner-skills/cdd-engine` major + `@oscaner-skills/osuperpowers` major，Breaking：gate 删除 + `--harness`/`--doc` 删除 + host-harness 收敛）；P5-design +v1.3（Approved — Plan done）；branch 收尾移交 finishing | [human] · Claude Opus 4.8 |
| v1.27 | 2026-09-10 | **四表状态回填（audit closeout）**：Phase inventory P1–P4/P6 Implementation plan 列 Pending → **Done**（PR #236/#237/#243/#244/#245 均已 merge）；P2 Design spec 列 Pending → **P2-design**；change-history v1.25/v1.26 降序回归修复（append 升序）—— 上报 #246 comment 5616300332（overall 四表状态与交付不同步 bug） | [human] · Claude Opus 4.8 |
