...
| P6 | P4 dogfood 修复：F1 progress.json 任务完成态/lastDispatchHead 回写 + F2 writing-plans 产出 `**Spec:**` 头行 + F3 `cdd review` status rollup 统一（docs 类型 warn/nit→APPROVED，CHANGES_REQUESTED 仅 blocker>0）+ F4 Bug C 扩展（implement 侧 handoff 缺失兜底/文档化）+ F5 stale-lexicon 守卫脚本化（validate/emit:check）+ F6 `.superpowers/docs-review/` 旧命名产物归档 | Pending | Pending | runner 在 APPROVED task-review 后回写 progress task status=complete + lastDispatchHead；writing-plans plan 头含 `**Spec:**` 链接；`cdd review --type spec\|plan` 全 warn/nit → APPROVED（无 blocker 不产出 CHANGES_REQUESTED，review 输出 status 与 severity 校验一致）；`pnpm run validate` 含 stale-lexicon 检查（豁免清单数据化）；doc-review workspace 无 `plan-review-*`/`spec-review-*` 旧产物；`pnpm run validate` 绿 | P4（hard：依赖 P4 交付的 report-target schema / `**Spec:**` 头 / 渲染器 / 双通道） |
...
P1 ────────────────────────────────┐ (P1→P4 hard) ─────────┘
P1 ─────────────────────────────────────────────── (soft) ──→ P5 (Gate 移除)
P4 ──→ P6 (P4 dogfood 修复：progress 回写 / Spec: 头 / status rollup / Bug C / 守卫脚本化 / workspace 卫生)
```
