...
| P3 | [#232](https://github.com/Oscaner/skills/issues/232#issuecomment-5553108929) | Enhancement V — report-issue file 时 comment 内嵌 GitHub tasklist checkboxes（phase 进度追踪；R/S/T/U 已 retrofit tasklist） |
| P3 | [#232](https://github.com/Oscaner/skills/issues/232#issuecomment-5553110014) | Enhancement W — finishing 完成 phase 时勾选对应 tasklist checkbox 为 done（与 Enh V 成对；U 的 P2 项已预勾） |
...
| P3 | Skills + 模板重构：Enh J report-issue session master issue + findings 以 comment 追加 + 永不 reopen + overall spec 模板 Issue inventory 附 comment URL（`#issuecomment-NNN` 锚点格式）；Enh K brainstorming explore-context 主动读取关联 issue comments（`gh issue view NNN --json body,comments`，fail-open）；**Enh R writing-plans `user-ok?`「Fix selected」死选项移除**；**Enh S cdd-engine review 模板 deferred 孤儿机制清理**（task-review.md 去 deferred 标记契约 + contract/progress/schema/docs 引用清理）；**Enh T fix handoff schema 加可选 `notes` 字段**；**Enh V report-issue file 时内嵌 GitHub tasklist**；**Enh W finishing 收官勾选对应 tasklist 为 done** | Pending | Pending | 同 session findings 全部 comment 追加到 session master issue，不新建独立 issue；closed issue 命中时创建新 comment + reference，永不 reopen；overall spec 模板 Issue ref 列支持 comment URL 锚点；本 overall 自身 Issue inventory 已按新格式填写；**report-issue `.md` body 模板 mirror P2 新增的 yml 表单字段（component / session-type，P2 已定 A 决策推迟至 P3）**；**writing-plans `user-ok?` 不再提供「Fix selected」选项（digraph `fix selected` 边移除）**；**review/task-review 模板不再标记 warn/nit 为 deferred（contract/progress/schema/docs 引用清零）**；**fix handoff schema 支持可选 `notes` 字段（T2 实测被拒两次 dispatch）**；**report-issue filing 内嵌 tasklist checkboxes + finishing 收官勾选 done（#232 comments 现含可追踪 tasklist）**；brainstorming 在 phase-within-program 模式下主动读取 overall Issue inventory 中所有 `#NNN` 的 comments | P1（hard）；P2（soft：串行管理约定，P3 与 P2 无技术强依赖） |
