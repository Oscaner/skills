...
14. **旧路径残留**：`grep -rnE "scripts/ci-validate\.mjs|scripts/lib/|scripts/gh-branch-rulesets|scripts/emit\.mjs|scripts/version-packages\.mjs|scripts/publish-vendor\.mjs|scripts/bump-submodule\.mjs|scripts/validate-marketplace\.mjs|scripts/validate-version-sync\.mjs" . --include="*.mjs" --include="*.json" --include="*.yml" --include="*.md" --exclude-dir=node_modules --exclude-dir=.git --exclude-dir=.superpowers --exclude-dir=docs/superpowers --exclude-dir=vendors` 为空（**排除 `vendors/`**：vendored 子模块内容不在本 phase 修改范围）。
15. **composite actions 实际使用 + 成功断言**：pr-validate.yml 明确 `uses: ./.github/actions/{setup,link-cdd-engine,validate,install-harness}`，并含 `run: node scripts/run.mjs smoke-cdd` 步骤（CI smoke 真实执行）；`install-harness` 在 hermetic HOME 运行后**断言 `$HOME` 下 init 产物存在**（具体路径清单由 plan 依 `configs/` 模板确定，至少 config/skills 一类）且 action exit 0；`link-cdd-engine` 断言 `command -v cdd-task` 存在。
