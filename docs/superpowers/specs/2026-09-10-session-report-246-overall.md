...
| P2 | cdd review Stopping ref 增内容指纹维度：spec/plan 内容实质演进即新 ref、可开新 review cycle（同 ref + blocker=0 仍禁止 re-run）；文档化 spec 演进重审路径 | P2-design（2026-09-11，v1.0） | [Pending] | 同一 spec/plan 文档内容修改后 `cdd review --round 2` 可开新 review cycle；内容未变仍 exit 3；无复制文档到新路径重审的双文件漂移 anti-pattern | 无 |
