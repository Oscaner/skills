...
it("review-mode 定稿注入 doc_hash：缺失 doc（mock 环境 ENOENT）→ 空串哨兵 + 内存返回值同步", async () => {
  ...
  const result = await runDocsTask({
    ...
    doc: "/repo/root/osuperpowers/specs/my-spec.md",   // 不存在 → hashFile "" 哨兵
