...
**根因**：`workspace = path.dirname(values.doc)` 将 handoff 写到文档同目录（如 `osuperpowers/specs/`）。
...
5. **Bug K**：`docs-task --mode review --template spec-review --doc docs/x.md` 写出 `.superpowers/docs-review/spec-review-1.json`（不再写到 `osuperpowers/specs/`）
