...
| P1 | 补充拍板（2026-09-13） | `.superpowers/standalone/` 无独立意义 → 并入 `cdd/<slug>`；不同 feature 可有不同 base-branch（base-branch.json per-slug） |
...
| P3 | req 7 | 清理冗余 cdd 命令；base-branch CLI 去 `--scope standalone` |
...
|---|---|---|---|---|---|---|
| P1 | **cdd-engine 服务化重构① · runtime 布局**：workspace 根 `.superpowers/cdd` → `.osuperpowers/cdd`（handoff-namespace.json#workspaceRoot / lifecycle / progress / registry 路径随迁）；standalone 并入 `cdd/<slug>`（STANDALONE_ROOT 移除，base-branch.json per-slug，不同 feature 独立 base）；`.superpowers/{docs-review, archive-*}` 残留清除；**存量 `.superpowers/cdd/*` 工作区处置**（17+ 历史程序 + lifecycle.json：P1 落地后 resolveWorkspace 按新根解析 `.osuperpowers/cdd/<slug>/`，旧根工作区**显式废弃**——保留转只读状态、不迁移不复写；本程序自身的 spec-review-1 已落在旧根，P1 后后续 phase 的 review/fix 在新根重新开 round——doc_hash 双签名使 Stopping 语义不因分根断裂）；gitignore / residue guard / smoke-cdd / engine tests / skills docs 路径 / report-issue 读取路径同步（`.superpowers/sdd` 保留——superpowers 所属，不在本程序迁移域） | [Pending] | [Pending] | 引擎运行期零 `.superpowers` 写入（superpowers 的 sdd 读取除外）；standalone 场景复用 `cdd/<slug>` 且 per-slug base-branch 独立；`.superpowers/{docs-review,archive-*}` 遗留已清除；**存量工作区处置清单完成**（废弃标记/只读语义落盘，无孤儿写）；`pnpm run validate` 13 块全绿 | 无（program 起点） |
...
| P3 | **cdd-engine 服务化重构② · 命令面与契约**：`cdd brief` 自包含入 implement（删独立命令 + 相对 `packages/cdd-engine/` 的 `lib/cli/brief.mjs`，保留 `lib/brief.mjs generateBrief` 供 run-task）；`cdd research` 移除（删 CLI + `lib/cli/research.mjs` + tests + SUBCOMMAND_USAGE【位于 `lib/cli/parse.mjs`】+ bin 注释【位于 `bin/cdd.mjs`】）；base-branch CLI 去 `--scope standalone`（并入 cdd slug）；usage / README CDD CLI 表同步 | [Pending] | [Pending] | `cdd --help` 子命令集合收敛为 implement/review/fix/base-branch；零 brief/research 残留引用（历史 plan/spec 文档除外）；engine 相关 tests 与命令面同步；`pnpm run validate` 全绿 | P1 ->(soft) P3（standalone 路径并入先行） |
...
```
P1 ->(soft) P3   (standalone 路径并入后，base-branch CLI 清理更顺)
...
phase program:
  ...
  4. 判断 phase size 是否合适、是否需子 phase 拆分？优先不拆分；无法抉择 → AskUserQuestion
     size 合适 → Run `/osuperpowers:writing-phase-spec`
