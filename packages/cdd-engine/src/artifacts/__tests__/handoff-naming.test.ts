...
import {
  handoffName, roundPattern, resolveNextRound, prevHandoffPath, tasksKey,
...
it("tasksKey: full list string, no range abbreviation (--tasks 1 → 1 · --tasks 1,2 → 1-2)", () => {
  expect(tasksKey([1])).toBe("1");
  expect(tasksKey([1, 2])).toBe("1-2");
  expect(tasksKey([3, 7, 12])).toBe("3-7-12");
});
...
it("prevHandoffPath: fix.task round1 无专属表项 → 回退 roundR（跨轮次同依赖）", () => {
  expect(prevHandoffPath("/ws", "fix", "task", 1, { tasks: "5" })).toBe("/ws/tasks-5-review-1.json");
  expect(prevHandoffPath("/ws", "fix", "task", 3, { tasks: "1-2" })).toBe("/ws/tasks-1-2-review-3.json");
});
