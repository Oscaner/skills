...
it("createEmptyProgress: plan parameter is used", () => {
  const p = ledger.create("/path/to/plan.md");
...
it("readProgressJSON: reads existing progress.json", () => {
  ...
  const data = ledger.create("/plan.md");
  data.timeoutCount = 5;
  data.tasks = [{ task: 1 }];
  ledger.write(dir, data);
  const p = ledger.read(dir);
...
it("migrateFromProgressMD: returns null when no progress.md", () => {
  ...
  expect(ledger.migrateFromProgressMD(dir)).toBe(null);
});
...
it("migrateFromProgressMD: parses timeoutCount from # timeoutCount: N", () => {
  ...
  const p = ledger.migrateFromProgressMD(dir);
...
it("migrateIfNeeded: progress.json exists → returns it (no migration)", () => {
  ...
  data.timeoutCount = 7;
  ledger.write(dir, data);
  const p = ledger.migrateIfNeeded(dir);
...
it("getRound: ledger.incrementRound('review') 后 round=2（rounds['review'] 归一键）", () => {
  const dir = tmpDir("prog-round-review-");
  expect(ledger.getRound(ledger.read(dir), 1, "review")).toBe(1);
  ledger.incrementRound(dir, 1, "review");
  expect(ledger.getRound(ledger.read(dir), 1, "review")).toBe(2);
...
describe("progress.ts group ledger (group is the unit)", () => {
  it("incrementRound group key → 单个 {group} 行，round 按组一份", () => {
    ...
    expect(ledger.getRound(ledger.read(dir), "1,2", "implement")).toBe(1);
    ledger.incrementRound(dir, "1,2", "implement");
    ledger.incrementRound(dir, "1,2", "review");
    const saved = JSON.parse(readFileSync(path.join(dir, "progress.json"), "utf8"));
    expect(saved.tasks).toEqual([{ group: "1,2", rounds: { implement: 1, review: 1 } }]);
    expect(ledger.getRound(ledger.read(dir), "1,2", "implement")).toBe(2);
    expect(ledger.getRound(ledger.read(dir), "1,2", "review")).toBe(2);
  });

  it("single-task group key 解析到 per-task 行（向后兼容：--tasks 1 与既有 task 行同面）", () => {
    const dir = tmpDir("prog-group-single-");
    ledger.incrementRound(dir, "1", "implement");
    const saved = JSON.parse(readFileSync(path.join(dir, "progress.json"), "utf8"));
    expect(saved.tasks).toEqual([{ task: 1, rounds: { implement: 1 } }]);
    // scalar 数字 key 与单组字符串 key 读到同一行
    expect(ledger.getRound(ledger.read(dir), "1", "implement")).toBe(2);
    expect(ledger.getRound(ledger.read(dir), 1, "implement")).toBe(2);
  });
  ...
  it("单组与多组互不串行：group 1,2 的 rounds 不落到 task 1 / task 2 行", () => {
    ...
    ledger.incrementRound(dir, "1,2", "implement");
    expect(ledger.getRound(ledger.read(dir), "1", "implement")).toBe(1);
    expect(ledger.getRound(ledger.read(dir), "2", "implement")).toBe(1);
    ledger.incrementRound(dir, "1", "implement");
    expect(ledger.getRound(ledger.read(dir), "1,2", "implement")).toBe(2);
    expect(ledger.getRound(ledger.read(dir), "1", "implement")).toBe(2);
  });
  ...
  it("scope 账本按组 keyed：seed/taskScopeBase 走 group 行", () => {
    ...
    expect(ledger.seedScopeBase(dir, "1,2", base)).toBe(base);
    expect(ledger.taskScopeBase(dir, "1,2")).toBe(base);
    // 单组 key 仍走 per-task 行
    expect(ledger.seedScopeBase(dir, "3", "b".repeat(40))).toBe("b".repeat(40));
    expect(ledger.taskScopeBase(dir, 3)).toBe("b".repeat(40));
  });
});
...
describe("progress.ts scope 账本（T27/spec T7.6）", () => {
  ...
  it("taskScopeBase: 账本缺失 → null；task 存在但无 scope_base → null；既有合法值 → 该值", () => {
    ...
    expect(ledger.taskScopeBase(dir, 7)).toBeNull();
    const data = ledger.read(dir);
    data.tasks = [{ task: 7, rounds: {} }];
    ledger.write(dir, data);
    expect(ledger.taskScopeBase(dir, 7)).toBeNull();
    data.tasks[0].scope_base = "a".repeat(40);
    ledger.write(dir, data);
    expect(ledger.taskScopeBase(dir, 7)).toBe("a".repeat(40));
  });
  ...
  it("taskScopeBase: 非法存量值（非 40-hex）→ null（账本视为缺失，回落 legacy 链）", () => {
    ...
    ledger.write(dir, data);
    expect(ledger.taskScopeBase(dir, 3)).toBeNull();
  });

  it("seedScopeBase: 首次 seed 落账本（创建 task 条目+写盘）；earliest-wins——重复 seed 不覆盖", () => {
    const dir = tmpDir("prog-scope-seed-");
    expect(ledger.seedScopeBase(dir, 1, "a".repeat(40))).toBe("a".repeat(40));
    // 第二次 seed（不同值）被拒 —— 既有合法值永远赢
    expect(ledger.seedScopeBase(dir, 1, "b".repeat(40))).toBe("a".repeat(40));
    expect(ledger.taskScopeBase(dir, 1)).toBe("a".repeat(40));
    const saved = JSON.parse(readFileSync(path.join(dir, "progress.json"), "utf8"));
    expect(saved.tasks[0]).toEqual({ task: 1, scope_base: "a".repeat(40) });
    // 非 40-hex 的 seed 输入 → 拒绝（不写）
    expect(ledger.seedScopeBase(dir, 1, "not-a-sha")).toBe("a".repeat(40));
  });
  ...
  it("seedScopeBase: 非法存量值可被首次合法 seed 覆盖（脏账本自愈），合法值仍 earliest-wins", () => {
    ...
    ledger.write(dir, data);
    expect(ledger.seedScopeBase(dir, 4, "c".repeat(40))).toBe("c".repeat(40));
    // 自愈后进入 earliest-wins：再 seed 不覆盖
    expect(ledger.seedScopeBase(dir, 4, "d".repeat(40))).toBe("c".repeat(40));
    expect(ledger.taskScopeBase(dir, 4)).toBe("c".repeat(40));
  });
  ...
  it("moveTaskScopeBaseEarlier: 严格更早的祖先（仍是 HEAD 祖先）→ 账本前移", async () => {
    ...
    ledger.seedScopeBase(dir, 2, c1);
    expect(await ledger.moveTaskScopeBaseEarlier(dir, 2, c0, repo, c2)).toBe(c0);
    expect(ledger.taskScopeBase(dir, 2)).toBe(c0);
  });

  it("moveTaskScopeBaseEarlier: 后裔/更晚提交被拒（非 current 祖先）；即使它是 HEAD 祖先", async () => {
    const { repo, c0, c1, c2 } = threeCommitRepo();
    const dir = tmpDir("prog-scope-move-desc-");
    ledger.seedScopeBase(dir, 2, c0);
    // c1 是 c2(HEAD) 的祖先、但相对账本值 c0 是后裔 → 被拒，账本不动
    expect(await ledger.moveTaskScopeBaseEarlier(dir, 2, c1, repo, c2)).toBe(c0);
    expect(ledger.taskScopeBase(dir, 2)).toBe(c0);
  });
  ...
  it("moveTaskScopeBaseEarlier: ==head 被拒（Reset/cheat 面）", async () => {
    ...
    ledger.seedScopeBase(dir, 2, c0);
    expect(await ledger.moveTaskScopeBaseEarlier(dir, 2, c2, repo, c2)).toBe(c0);
    expect(ledger.taskScopeBase(dir, 2)).toBe(c0);
    // c1 已是 current 自身 → 相等被拒（无变化）
    ledger.seedScopeBase(dir, 5, c1);
    expect(await ledger.moveTaskScopeBaseEarlier(dir, 5, c1, repo, c2)).toBe(c1);
  });
  ...
  it("moveTaskScopeBaseEarlier: 非祖先伪造（另一个仓库的 commit，与其 init 同形）被拒——祖先校验拒伪造", async () => {
    ...
    ledger.seedScopeBase(dir, 2, c1);
    expect(await ledger.moveTaskScopeBaseEarlier(dir, 2, forgeHead, repo, c2)).toBe(c1);
    expect(ledger.taskScopeBase(dir, 2)).toBe(c1);
  });
