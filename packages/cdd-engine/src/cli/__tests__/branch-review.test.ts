...
describe("branch-review unparseable-handoff e2e", () => {
  async function runBranchReviewWithFakeCli(
    opts: { handoffBody: string },
    deps: {
      dir: string;
      planPath: string;
      base: string;
  ...
  it("corrupt previous-round handoff fails open (CDD_INFO, no Convergence lock), next round proceeds", async () => {
    ...
    const writes: string[] = [];
    process.stderr.write = (chunk: unknown, ..._a: unknown[]) => {
    ...
    try {
      const exitCode = await runBranchReviewWithFakeCli(
        {
          handoffBody:
            '{"tasks":[1],"phase":"branch-review","status":"APPROVED","commits":{"base":"' +
            base +
            '","head":"' +
            head +
            '"},"findings":[],"artifacts":{}}',
