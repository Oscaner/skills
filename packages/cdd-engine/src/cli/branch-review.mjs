// packages/cdd-engine/src/cli/branch-review.mjs — `cdd review --type branch`（原 branch-review bin 动作体）。
...
export async function runBranchReview(opts) {
  ...
  // root 单一权威（src/infra/root.mjs）：initRoot() 已在 bin 的 preAction 内完成「非 git 仓 → BLOCKED exit 1」
  ...
  if (DRY_RUN()) {
    ...
    // counters 第 5 行 —— 与 h1FourLines / h1FromHandoff 同源于 src/artifacts/progress.mjs#h1CountersLine
