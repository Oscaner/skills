// packages/cdd-engine/src/dispatch/docs.mjs（ex lib/runner/run-docs.mjs）— lightweight runner for cdd review/fix
...
// P4 §2.4.1：root 由调用方注入（root 单一权威 src/infra/root.mjs）；本文件不自算第二权威。
...
import { finalizeHandoff, persistFinalized } from "../artifacts/handoff/finalize.mjs";
import { loadRegistry, checkHarness, REG_PATH } from "../infra/registry.mjs";
import { loadHandoffSchema, validateHandoffSchema, recoverHandoff } from "../rules/schema.mjs";
...
// REG_PATH 统一由 src/infra/registry.mjs 导出（spec §2.3 深度派生常数专项：run-docs 不再自算第二来源）。
...
export async function runDocsTask({
  ...
  // repoRoot 注入缝：调用方（CLI 层）经单一 root 权威 src/infra/root.mjs 派生后传入；
  ...
  // 注入值优先；未注入 → 取 engine 单根（src/infra/root.mjs）。求值在 dry-run 早退之后：
  ...
  if (!sv.valid) {
    ...
    // 恢复单点 = src/rules/schema.mjs#recoverHandoff（归一化 → 重校验，最多一轮；违规键名后缀与
