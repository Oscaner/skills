// packages/cdd-engine/src/dispatch/task.mjs（ex lib/runner/run-task.mjs）— CDD per-task runner (Node port of cdd_run_task).
...
// Final exit delegated to src/infra/exit.mjs (unified exit point, no inline process.exit).
...
import { exitOk, exitBlocked, exitCliMissing, exitWithCode, ExitRequested } from "../infra/exit.mjs";
...
import { readProgressJSON, writeProgressJSON, migrateIfNeeded, getRound, incrementRound, incrementRecovery, h1CountersLine } from "../artifacts/progress.mjs";
import { briefPath } from "../artifacts/base-branch.mjs";
import { validateHandoffSchema, recoverHandoff } from "../rules/schema.mjs";
import { FAILURE_CATEGORIES, counterFor } from "../rules/failure.mjs";
...
//   prefix 值经 registry resolveInjection（entry.prefix[op][type?]）解析（见 src/infra/invoke.mjs）。
const INVOKE_PARAMS = {
...
//（failure_category 赋值点 / 计数器 increment 落点）一律经 src/rules/failure.mjs 承重取值 ——
...
// root authority (src/infra/root.mjs — the engine's only cwd conversion point), the effective plan from
...
// —— 与 artifacts/handoff/naming.mjs#resolveWorkspace(doc, root)（纯路径派生）重名消解：本函数含
...
export async function runTask(harness, taskNum, opts = {}) {
  ...
  // cdd-engine is self-contained: pluginRoot() = src/render/templates.mjs PKG_ROOT =
  ...
  if (dryRun) {
    ...
    // 子进程 cwd = 注入的 root（唯一 root 权威 src/infra/root.mjs；本函数不直读启动 cwd，也无第二注入缝）。
  ...
  if (mode !== "implement") {
    ...
    if (existingHandoff) {
      ...
      if (!sv.valid) {
        ...
        // 归一化 → 重校验（最多一轮，不循环）——恢复单点是 src/rules/schema.mjs#recoverHandoff
