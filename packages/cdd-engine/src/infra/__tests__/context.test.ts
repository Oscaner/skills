...
describe("context-contract canonical 承重", () => {
  it("timeout 默认值取自 canonical", () => {
    ...
    expect(invoker.resolveTerminationConfig("task").budgetMs).toBe(c.timeouts.defaults.task);
    expect(invoker.resolveTerminationConfig("review").budgetMs).toBe(c.timeouts.defaults.review);
  });
  it("统一 resolver：seam overrides 优先、stall 节奏同读 canonical timeouts.liveness、未知 mode → budget 缺省", () => {
    ...
    expect(invoker.resolveTerminationConfig("task", { budgetMs: 42 }).budgetMs).toBe(42);
    expect(invoker.resolveTerminationConfig("review", { budgetMs: 1000 }).budgetMs).toBe(1000);
    ...
    expect(invoker.resolveTerminationConfig("task", { idleWindowMs: 500 }).idleWindowMs).toBe(500);
