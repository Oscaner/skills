...
// 机制位文件清单：src 全 `.ts`（P6 后 src 恒真零 .mjs），排除 `__tests__/`——测试内
// 的 env 直读/PATH 遮蔽/tmp cwd 是合法测试编排，不是机制通道回渗（validate 5c 同口径）。
function mechanismFiles() {
  const files = execaSync("bash", ["-lc",
    `find packages/cdd-engine/src -name '*.ts' ! -path '*/__tests__/*'`,
  ], { cwd: REPO_ROOT, encoding: "utf8" }).stdout.trim().split("\n").filter(Boolean);
...
describe("engine env 面收口", () => {
  ...
  it("零 spread 注入（{ ...process.env, … }）", () => {
    const offenders = mechanismFiles().filter((f) => readFileSync(f, "utf8").includes("...process.env"));
    expect(offenders).toEqual([]);
  });
  it("六个已删键名零命中（PLAN_FILE 含在内——故 PLAN_LINE 由显式 planFile 参数派生）", () => {
    const offenders = mechanismFiles().filter((f) => DELETED.some((k) => readFileSync(f, "utf8").includes(k)));
    expect(offenders).toEqual([]);
  });
});
