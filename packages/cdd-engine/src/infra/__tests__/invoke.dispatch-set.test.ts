...
describe("C5 — dispatch-set composition (invoke string / cwd / env)", () => {
  it("assembly = [registry prefix, prompt, suffix] joined on newlines (C1: prefix precedes the prompt)", () => {
    ...
    expect(invoker.promptArgText("", "hello", "")).toBe("hello"); // no prefix/suffix → prompt verbatim
  });

  it("buildInvokeArgs = invoke spec words + prompt arg last", () => {
    expect(invoker.buildInvokeArgs("-p --output-format text", "the prompt")).toEqual([
      "-p",
      "--output-format",
      "text",
      "the prompt",
  ...
  it("same (harness, op, type) + same inputs → byte-identical dispatch set (byte-equal args/cwd/env)", () => {
    ...
    const set2 = invoker.composeDispatchSet(entry, { op: "implement" }, "prompt A", "/wsA", {
      PATH: "/usr/bin",
      HOME: "/h",
    });
  ...
  it("(op, type) granularity resolves distinct prefixes deterministically (registry-driven, zero env sway)", () => {
    ...
    expect(registry.resolveInjection(entry, "review", "task")).toBe(
      registry.resolveInjection(entry, "review", "task"),
    );
    expect(registry.resolveSuffix(entry, "implement")).toBe("");
  });
