...
export class EngineInvoker {
  ...
  promptArgText(prefix: string, prompt: string, suffix: string): string {
    return [prefix, prompt, suffix].filter(Boolean).join("\n");
  }

  /** invoke-spec words + the prompt arg last (the exact legacy args shape). */
  buildInvokeArgs(invokeSpec: string, promptArg: string): string[] {
  ...
  composeDispatchSet(
    ...
    const p = this.#registry.resolveInjection(entry, op, type);
    const s = this.#registry.resolveSuffix(entry, op, type);
    const promptArg = this.promptArgText(p, prompt, s);
    return { cli: entry.cli, args: this.buildInvokeArgs(entry.invoke, promptArg), cwd, env };
  }
  ...
  async invokeCli(
    ...
    const set = this.composeDispatchSet(entry, params, prompt, cwd, env);
  ...
  async invokeCliWithRetry(
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
      const result = await this.invokeCli(entry, prompt, params, env, cwd, termination);
