...
describe("rules/convergence.ts — blockerCount", () => {
  ...
  it("缺 severity 的 finding 不计", () => {
    expect(blockerCount({ findings: [{ summary: "no severity" }, { severity: "blocker" }] })).toBe(
      1,
    );
  });
});
...
describe("rules/convergence.ts — reviewConvergenceGuard（Review Convergence 判定）", () => {
  ...
  it("APPROVED + blocker 0 → exit 3（停）", () => {
    expect(
      exitCodeOf(() =>
        reviewConvergenceGuard({ status: "APPROVED", findings: [] }, "task", 1, "plan"),
      ),
    ).toBe(3);
  });
  ...
  it("SP-4: BLOCKED/TIMEOUT 失败轮（findings:[]）→ 不停（须可重派）", () => {
    expect(
      exitCodeOf(() =>
        reviewConvergenceGuard({ status: "BLOCKED", findings: [] }, "task", 1, "plan"),
      ),
    ).toBe(null);
    expect(
      exitCodeOf(() =>
        reviewConvergenceGuard({ status: "TIMEOUT", findings: [] }, "task", 1, "plan"),
      ),
    ).toBe(null);
  });
