...
describe("rules/convergence.ts — blockerCount", () => {
  ...
  it("findings 缺失 / 空 → 0", () => {
    expect(convergence.blockerCount({})).toBe(0);
    expect(convergence.blockerCount({ findings: [] })).toBe(0);
    expect(convergence.blockerCount(null)).toBe(0);
  });

  it("缺 severity 的 finding 不计", () => {
    expect(
      convergence.blockerCount({ findings: [{ summary: "no severity" }, { severity: "blocker" }] }),
    ).toBe(1);
  });
});
...
describe("rules/convergence.ts — reviewConvergedError（reason 分场景消息）", () => {
  it("缺省文案含 Review Convergence: 与 /blocker=0/ 不变量", () => {
    expect(convergence.reviewConvergedError("task", 2, "plan").message).toMatch(
      /Review Convergence:/,
    );
    expect(convergence.reviewConvergedError("task", 2, "plan").message).toContain(
      "already blocker=0",
    );
  });

  it("reason=legacy → pre-content-hash review handoff 指引", () => {
    const e = convergence.reviewConvergedError("spec", 1, "doc.md", { reason: "legacy" });
  ...
  it("reason=unchanged → doc content unchanged since round N", () => {
    const e = convergence.reviewConvergedError("plan", 3, "plan.md", { reason: "unchanged" });
...
describe("rules/convergence.ts — convergedExit3（ExitRequested.code = 3）", () => {
  it("抛 ExitRequested(3)，stderr 含错误消息", () => {
    const code = exitCodeOf(() => convergence.convergedExit3("task", 1, "plan", "stale blocker"));
    expect(code).toBe(3);
  });
});
...
describe("rules/convergence.ts — reviewConvergenceGuard（Review Convergence 判定）", () => {
  ...
  it("prev null / undefined → 不停", () => {
    expect(exitCodeOf(() => convergence.reviewConvergenceGuard(null, "task", 1, "plan"))).toBe(
      null,
    );
  });
  ...
  it("SP-4: BLOCKED/TIMEOUT 失败轮（findings:[]）→ 不停（须可重派）", () => {
    expect(
      exitCodeOf(() =>
        convergence.reviewConvergenceGuard({ status: "BLOCKED", findings: [] }, "task", 1, "plan"),
      ),
    ).toBe(null);
    expect(
      exitCodeOf(() =>
        convergence.reviewConvergenceGuard({ status: "TIMEOUT", findings: [] }, "task", 1, "plan"),
      ),
    ).toBe(null);
  });
    ...
    expect(
      exitCodeOf(() =>
        convergence.reviewConvergenceGuard(
          { status: "APPROVED", findings: [{ severity: "blocker" }] },
          "task",
          1,
          "plan",
        ),
