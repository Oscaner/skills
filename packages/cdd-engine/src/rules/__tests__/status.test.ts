...
describe("deriveTaskState — six-state convergence", () => {
  ...
  it("in-flight: implement carrier BLOCKED (non-APPROVED, non-dead) → re-dispatch implement", () => {
    const ws = workspace(EMPTY_PROGRESS);
    writeHandoff(ws, "tasks-1-implement.json", { task: 1, phase: "implement", status: "BLOCKED", failure_category: "CONTRACT_VIOLATION", findings: [], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("in-flight");
  });

  it("needs-review: implement APPROVED + no review on record", () => {
    const ws = workspace(EMPTY_PROGRESS);
    writeHandoff(ws, "tasks-1-implement.json", { task: 1, phase: "implement", status: "APPROVED", findings: [], artifacts: {} });
  ...
  it("needs-fix: review CHANGES_REQUESTED + no fix on record", () => {
    ...
    writeHandoff(ws, "tasks-1-review-1.json", { task: 1, phase: "review", status: "CHANGES_REQUESTED", findings: [{ severity: "blocker" }], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("needs-fix");
  });

  it("needs-fix: review BLOCKED (engine BLOCKED channel) likewise awaits the fix", () => {
    const ws = workspace(ROUNDS({ review: 1 }));
    writeHandoff(ws, "tasks-1-review-1.json", { task: 1, phase: "review", status: "BLOCKED", failure_category: "UNVERIFIABLE", findings: [], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("needs-fix");
  });
  ...
  it("needs-fix: fix BLOCKED (fix attempted but not landed) → fix again", () => {
    ...
    writeHandoff(ws, "tasks-1-review-1.json", { task: 1, phase: "review", status: "CHANGES_REQUESTED", findings: [{ severity: "blocker" }], artifacts: {} });
    writeHandoff(ws, "tasks-1-fix-1.json", { task: 1, phase: "fix", status: "BLOCKED", blocker: "fix could not land", findings: [], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("needs-fix");
  });
  ...
  it("needs-re-review (T14 explicit): review not approved → fix APPROVED, no subsequent review", () => {
    const ws = workspace(ROUNDS({ review: 1, fix: 1 }));
    writeHandoff(ws, "tasks-1-review-1.json", { task: 1, phase: "review", status: "CHANGES_REQUESTED", findings: [{ severity: "blocker" }], artifacts: {} });
    writeHandoff(ws, "tasks-1-fix-1.json", { task: 1, phase: "fix", status: "APPROVED", findings: [], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("needs-re-review");
  });

  it("resume-pending: implement TIMEOUT dead round → resume or discard", () => {
    const ws = workspace(EMPTY_PROGRESS);
    writeHandoff(ws, "tasks-1-implement.json", { task: 1, phase: "implement", status: "TIMEOUT", failure_category: "TIMEOUT", findings: [], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("resume-pending");
  });

  it("resume-pending: implement EXECUTION_FAILURE (status BLOCKED + category)", () => {
    const ws = workspace(EMPTY_PROGRESS);
    writeHandoff(ws, "tasks-1-implement.json", { task: 1, phase: "implement", status: "BLOCKED", failure_category: "EXECUTION_FAILURE", blocker: "cli exited 1", findings: [], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("resume-pending");
  });

  it("resume-pending: review dead round (review TIMEOUT)", () => {
    const ws = workspace(ROUNDS({ review: 1 }));
    writeHandoff(ws, "tasks-1-review-1.json", { task: 1, phase: "review", status: "TIMEOUT", failure_category: "TIMEOUT", findings: [], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("resume-pending");
  });

  it("complete: latest review APPROVED (the existing writeback semantics)", () => {
    const ws = workspace(ROUNDS({ review: 1 }));
    writeHandoff(ws, "tasks-1-review-1.json", { task: 1, phase: "review", status: "APPROVED", findings: [], artifacts: {} });
  ...
  it("complete (T30 flip): review APPROVED → subsequent fix is the legal terminal, NOT a re-review trigger", () => {
    ...
    const ws = workspace(ROUNDS({ review: 1, fix: 1 }));
    writeHandoff(ws, "tasks-1-review-1.json", { task: 1, phase: "review", status: "APPROVED", findings: [{ severity: "warn" }], artifacts: {} });
    writeHandoff(ws, "tasks-1-fix-1.json", { task: 1, phase: "fix", status: "APPROVED", findings: [], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("complete");
  });
  ...
  it("complete (T30 flip): review APPROVED + fix that did not land still converges on the review verdict", () => {
    const ws = workspace(ROUNDS({ review: 1, fix: 1 }));
    writeHandoff(ws, "tasks-1-review-1.json", { task: 1, phase: "review", status: "APPROVED", findings: [], artifacts: {} });
    writeHandoff(ws, "tasks-1-fix-1.json", { task: 1, phase: "fix", status: "BLOCKED", blocker: "fix aborted", findings: [], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("complete");
  });
  ...
  it("complete (T30 terminal carve-out): a DEAD fix after an APPROVED review is never consulted — not resume-pending", () => {
    ...
    const ws = workspace(ROUNDS({ review: 1, fix: 1 }));
    writeHandoff(ws, "tasks-1-review-1.json", { task: 1, phase: "review", status: "APPROVED", findings: [{ severity: "warn" }], artifacts: {} });
    writeHandoff(ws, "tasks-1-fix-1.json", { task: 1, phase: "fix", status: "TIMEOUT", failure_category: "TIMEOUT", findings: [], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("complete");
    writeHandoff(ws, "tasks-1-fix-1.json", { task: 1, phase: "fix", status: "BLOCKED", failure_category: "EXECUTION_FAILURE", blocker: "cli exited 1", findings: [], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("complete");
  });
  ...
  it("needs-fix (T30 discriminator): a fix pre-dating the latest review does not re-review it", () => {
    ...
    writeHandoff(ws, "tasks-1-review-1.json", { task: 1, phase: "review", status: "CHANGES_REQUESTED", findings: [{ severity: "blocker" }], artifacts: {} });
    writeHandoff(ws, "tasks-1-fix-1.json", { task: 1, phase: "fix", status: "APPROVED", findings: [], artifacts: {} });
    writeHandoff(ws, "tasks-1-review-2.json", { task: 1, phase: "review", status: "CHANGES_REQUESTED", findings: [{ severity: "blocker" }], artifacts: {} });
    expect(deriveTaskState(ws, 1)).toBe("needs-fix");
  });
...
describe("derivePlanVerdict — plan completion verdict (`### Task N:` set ↔ six-state table)", () => {
  it("all-complete: every plan task complete → plan done", () => {
    ...
    for (const n of [1, 2]) {
      writeHandoff(ws, `tasks-${n}-review-1.json`, { task: n, phase: "review", status: "APPROVED", findings: [], artifacts: {} });
    }
  ...
  it("pending-with-reasons: mixed states → complete count + per-task six-state reasons", () => {
    ...
    writeHandoff(ws, "tasks-2-review-1.json", { task: 2, phase: "review", status: "CHANGES_REQUESTED", findings: [{ severity: "blocker" }], artifacts: {} });
    writeHandoff(ws, "tasks-2-fix-1.json", { task: 2, phase: "fix", status: "APPROVED", findings: [], artifacts: {} });
    const v: PlanVerdict = derivePlanVerdict(plan, ws, extractTasks);
