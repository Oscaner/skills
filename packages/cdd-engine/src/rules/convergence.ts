// packages/cdd-engine/src/rules/convergence.ts — Review Convergence guard cluster (Task 5 rules-layer
// rebuild; ex the cluster that cli/shared.mjs hosted). Self-contained per the closure-completeness
// ownership rule: reviewConvergenceGuard calls convergedExit3 + blockerCount + reviewConvergedError, so
...
export function convergedExit3(
  type: string,
  round: number,
  ref: string,
...
export function reviewConvergenceGuard(
  ...
  if (!incomplete && prev && prev.status === "APPROVED" && blockerCount(prev) === 0) {
    convergedExit3(type, round, ref, prev?.blocker, opts);
  }
}
