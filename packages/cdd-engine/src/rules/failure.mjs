// packages/cdd-engine/src/rules/failure.mjs（ex lib/failure.mjs）— 失败类目 canonical（templates/failure-categories.json）的运行期读取点
// （与 src/infra/context.mjs 同形：纯读、零落盘）。六类名与语义的唯一声明点 = canonical 静态契约数据；
