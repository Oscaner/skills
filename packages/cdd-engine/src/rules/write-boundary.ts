...
// belongs to the review layer (template-contract scope-composition axis → normal finding → fix loop),
// while the execution layer only keeps the books. implement rounds materialize no changes[] (the
// return block is their only surface) → the diff fileset is recorded verbatim as the ledger origin,
// zero warn. No usable commits.base → skipped (docs-family carries commits{base,head} since T5 — its
...
export class ChangedSurfaceAuditor {
  ...
  async reconcileChangedSurface(
    ...
    if (!head) return null;
    const diffFiles = await this.#git.diffNameOnly(repoRoot ?? "", base, head);
