...
**This mode does not write a handoff.** The runner materializes `task-{{TASK_NUMBER}}-implement.json` from your H1 four lines + the brief's `TASK_BASE` + `git HEAD`. The runner is the single authority for `commits` and for re-emitting your H1.
...
3. Write a full implementer report to the path named in the brief (typically `{{TASK_WORKSPACE}}/task-{{TASK_NUMBER}}-report.md`).
4. Write `{{TASK_WORKSPACE}}/task-{{TASK_NUMBER}}-test-evidence.json` with at least `command`, `exit_code`, `passed`, and `warnings_count` (include `behavior_change` when applicable).
...
After you exit, the runner reads `{{TASK_WORKSPACE}}/task-{{TASK_NUMBER}}-test-evidence.json`:
