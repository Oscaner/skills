...
This file provides guidance to Claude Code when working in the `packages/engineering/` plugin directory.
...
6. `packages/<plugin>/.claude-plugin/plugin.json` — generated per-plugin Claude manifest, e.g. [packages/superpowers-overrides/.claude-plugin/plugin.json](../superpowers-overrides/.claude-plugin/plugin.json). Registers skills by relative directory path.
...
The [superpowers-overrides](../superpowers-overrides/) plugin is the **trigger router** — it ships no skill bodies. The override skills live in [engineering](./skills/). Each `os-*` orchestrator skill follows a fixed shape:
...
Detail (pending-state contract, fail-open, shell allowlist) → [cross-harness-overrides.md](../superpowers-overrides/docs/cross-harness-overrides.md).
...
2. Add a target row to [packages/superpowers-overrides/overrides.manifest.json](../superpowers-overrides/overrides.manifest.json) mapping the upstream trigger to `engineering:<name>` (source `../engineering/skills/<name>`), then run `pnpm run emit` (regenerates `bin/prompt-expansion.mjs`, the cursor hooks, and `build/generated/*` via the unified `scripts/emit.mjs`). Do **not** hand-edit the hook script.
