...
test("appendLedger: deferred roll-up 行（只聚合 deferred:true）", () => {
  const dir = tmpDir();
...
test("appendLedger: 无 deferred → review clean", () => {
  const dir = tmpDir();
  const ledger = path.join(dir, "progress.md");
  writeFileSync(ledger, "# CDD ledger — plan: /x/plan.md\n");
  ...
  const content = readFileSync(ledger, "utf8");
  assert.ok(content.endsWith("Task 1: complete (commits base1..head1, review clean)\n"), `got: ${content}`);
});

test("appendLedger: 追加不覆盖 + base/head 截断 7 位", () => {
  const dir = tmpDir();
  const ledger = path.join(dir, "progress.md");
  ...
  const content = readFileSync(ledger, "utf8");
  assert.ok(content.includes("Task 1: complete (seed)"), "前置行保留（追加语义）");
  assert.ok(content.endsWith("Task 2: complete (commits 1234567..abcdef0, review clean)\n"), `got: ${content}`);
});

test("writePlanConstraints: 提取 ## Global Constraints 段（到下一个 ## 标题止，保留段内空行）", () => {
  const dir = tmpDir();
...
test("writePlanConstraints: --- 终止 + 已存在输出不覆盖", () => {
  const dir = tmpDir();
  const plan = path.join(dir, "plan.md");
  ...
  writeFileSync(plan, "## Global Constraints\n\nCHANGED\n");
  writePlanConstraints(plan, out);
  assert.equal(readFileSync(out, "utf8"), firstWrite);
  assert.equal(readFileSync(out, "utf8"), "\naaa\n");
});
