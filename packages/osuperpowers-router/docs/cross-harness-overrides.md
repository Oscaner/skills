...
Naming evolved across releases: v1 emit model → v2 `-overrides` suffix → v3 `spor-*` prefix → **v4 trigger router** (current). v3's `spor-*` skill bodies were deleted; **osuperpowers-router now ships no skill bodies** and routes to osuperpowers targets. See [CHANGELOG.md](../CHANGELOG.md) entries `6.2.0-overrides.3` through `6.2.0-overrides.6`.
...
## Solution (v4 — trigger router + osuperpowers)
...
**CI:** `pnpm run emit:check` checks generator drift; `tests/validate-overrides-build.mjs` validates the router + osuperpowers engine.
...
Cross-harness PreToolUse enforcement for CDD orchestrator sessions (Claude Code, Cursor, and the other gated harnesses — per-harness install → [docs/gate-install.md](../../../docs/gate-install.md)). The gate ships with **osuperpowers** — the overrides plugin is a trigger router and carries no gate hooks. The gate surface is **Node** (P4b): one decision core + one adapter per harness.
  ...
  "targets": [
    {
      ...
      "source": "../osuperpowers/skills/brainstorming"
    }
  ]
}
```
...
Regenerate after editing `overrides.manifest.json`, osuperpowers skills, or generator templates.

## Plugin discovery fallback (Cursor)

Skills ship under `packages/osuperpowers/skills/` in the plugin tree. After marketplace install, verify the osuperpowers skills (12 emitters + init) appear in the agent skills list.
