...
Read upstream `superpowers:receiving-code-review` SKILL.md as the process baseline **when available** (resolution priority + unavailability fallback same as [Rule: Read Upstream](../os-brainstorming/SKILL.md#rule-read-upstream)). **Read, not Skill-invoke**.
