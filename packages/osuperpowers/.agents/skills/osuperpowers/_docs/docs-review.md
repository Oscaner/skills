...
> Task-review uses Fix Loop in `cli-driven-development/SKILL.md`. Branch-review uses `cli-driven-development` + `node docs-task` (--mode review --template branch-review).
...
**CLI review:** each pass is an independent `node docs-task` invocation (stateless fresh nested session).
...
Path convention (enforced by docs-task engine — `node docs-task --mode review --doc <path>`):
