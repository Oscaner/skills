...
> Task-review uses Fix Loop in `cli-driven-development/SKILL.md`. Branch-review uses `cli-driven-development` + `cdd review --type branch`.
...
**CLI review:** each pass is an independent `cdd review --type spec|plan` invocation (stateless fresh nested session).
...
**`run-review`**
- Do: Execute one full review. CDD → `cdd review --type task`; doc-review → `cdd review --type spec|plan` (D1/D2/D3 passes). Count blockers from findings.
...
- Do: Pass all findings (blocker + warn + nit) to fix. CDD → `cdd fix --type task`; doc-review → `cdd fix --type spec|plan --doc <path> --findings <review-N-handoff-path>` (the `--findings` flag is required so {{FINDINGS}} in fix templates resolves to actual findings; without it the fix agent receives nothing to act on). Fix agent writes handoff with schema validation.
