...
| I5 | **Review Stopping** — re-run driven only by blockers; stop only when re-review output (docs-task CLI) shows 0 blockers for that pass — fixing locally and declaring blocker=0 without re-running docs-task on that pass is insufficient; no new docs-task call to obtain warn/nit (read from already-captured output of the current review cycle). |
