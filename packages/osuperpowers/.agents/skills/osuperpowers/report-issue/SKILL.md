...
- **Channel → kind mapping** (derivation for report-meta `kind`): program channel → `program`; session channel + CDD workspace slug present → `consumer-cdd`; session channel with no workspace (standalone run) → `standalone`. Every finding comment and master body carries exactly this resolved `kind` (I7).
