# osuperpowers -- CLAUDE.md

This file provides guidance to Claude Code when working in the `packages/osuperpowers/` plugin directory.
...
6. `packages/<plugin>/.claude-plugin/plugin.json` -- generated per-plugin Claude manifest, e.g. [packages/osuperpowers-router/.claude-plugin/plugin.json](../osuperpowers-router/.claude-plugin/plugin.json). Registers skills by relative directory path.
...
Detail (pending-state contract, fail-open, shell allowlist) --> [cross-harness-overrides.md](../osuperpowers-router/docs/cross-harness-overrides.md).

## Language architecture

**English-primary, Chinese-mirror.** Every source file has an English version as the authoritative source. Chinese (`.zh-CN.md`) files are human-readable mirrors for Chinese users; AI harnesses always read the English source and never treat `.zh-CN.md` as authoritative.

- `skills/*/SKILL.md` — edit here; this is what the harness loads.
- `skills/*/SKILL.zh-CN.md` — Chinese mirror; may drift after English edits; update separately in a dedicated translation pass.
- `docs/*.md` — English source for cross-cutting reference docs.
- `docs/*.zh-CN.md` — Chinese mirror; same drift rule.
- `.agents/skills/*/SKILL.md` and `.agents/skills/*/SKILL.zh-CN.md` — **never edit directly**; regenerated by `pnpm run emit` from the `skills/` source tree.

When an implementation task rewrites an English source file, explicitly defer `.zh-CN.md` updates in the task handoff findings (deferred, not a blocker). When a translation pass targets a `.zh-CN.md`, the English source must not change in the same task.
...
See [cdd-reference.md](docs/cdd-reference.md) H6 for full gate details, and `bin/utils/skills-probe.mjs` / `bin/utils/skills-probe.config.mjs` for implementation.
