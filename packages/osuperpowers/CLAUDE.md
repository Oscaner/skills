...
Two strategies apply depending on file type:

**Strategy A — English-primary + zh-CN mirror** (`skills/*/SKILL.md` and `docs/*.md`):

- `skills/*/SKILL.md` — English only; this is what the harness loads. No Chinese content.
- `skills/*/SKILL.zh-CN.md` — Chinese mirror; must be updated **in the same task** as the English source edit.
- `docs/*.md` — English only source for cross-cutting reference docs.
- `docs/*.zh-CN.md` — Chinese mirror; must be updated in the same task.
- `.agents/skills/*/SKILL.md` and `.agents/skills/*/SKILL.zh-CN.md` — **never edit directly**; regenerated by `pnpm run emit`.

**Strategy B — Chinese-primary, no mirror** (`docs/superpowers/specs/*.md` and `docs/superpowers/plans/*.md`):

Internal developer documents written in Chinese. No `.zh-CN.md` mirror is needed or maintained.
