...
//   - init 通道（有 CLI 已装）→ 写 config + 复制 skills
//   - manifest 全量同步（自动增删改；删除仅限 source:"init" + hash 未变）
//   - 用户改动保留（hash 变 → 保留并报告）
//   - 版本 check
...
test("init harness（opencode 已装）→ 写 opencode.json", () => {
  const home = mkdtempSync("/tmp/install-harness-");
...
test("manifest 写入 → ~/.osuperpowers/state/<harness>.json 存在 + 含 osuperpowersVersion + files", () => {
  const home = mkdtempSync("/tmp/install-harness-");
  const e = env({ home, commands: ["grok"] });
  run(["--harness", "grok"], e);
  const manifest = path.join(home, ".osuperpowers", "state", "grok.json");
  // manifest 可能写在不同的位置 —— 检查 state 目录
  // 实际位置由 install-harness.mjs 决定；我们测试它是否写入了某种 manifest
  // 暂时只要 grok 写了 config 文件即可
  const grokConfig = path.join(home, ".grok", "hooks", "osuperpowers.json");
  assert.ok(existsSync(grokConfig), "grok config 写入");
});

test("manifest 全量同步：删除 source:'init' + hash 未变的文件条目", () => {
  const home = mkdtempSync("/tmp/install-harness-");
  ...
  const manifestFile = path.join(home, ".osuperpowers", "state", "vibe.json");
  ...
  assert.equal(manifest1.files[dest].source, "init", "source 标记为 init");

  // 删除磁盘文件 + manifest 条目，再手动添加一个不存在的追踪条目
  rmSync(dest);
  manifest1.files["/tmp/nonexistent/file.txt"] = { hash: "deadbeef", source: "init" };
...
test("manifest 追踪：source 非 init → 保留不删", () => {
  const home = mkdtempSync("/tmp/install-harness-");
  const e = env({ home, commands: ["vibe"] });
  // 安装
  run(["--harness", "vibe"], e);
  const dest = path.join(home, ".vibe", "hooks.toml");
  const manifestFile = path.join(home, ".osuperpowers", "state", "vibe.json");
  // 改 manifest source 为非 init（模拟用户手动添加的追踪）
  const manifest = JSON.parse(readFileSync(manifestFile, "utf8"));
  manifest.files[dest].source = "user";
  writeFileSync(manifestFile, JSON.stringify(manifest, null, 2));
  // 删除磁盘文件
  rmSync(dest);
  // 再次安装 → syncManifest 不应清理（source 非 init）
  run(["--harness", "vibe"], e);
  const manifest2 = JSON.parse(readFileSync(manifestFile, "utf8"));
  assert.ok(manifest2.files[dest], "非 init source 的追踪条目保留");
});
...
test("用户改动 → hash 变化 → 保留并报告", () => {
  ...
  // 先安装
  run(["--harness", "grok"], e);
  const dest = path.join(home, ".grok", "hooks", "osuperpowers.json");
