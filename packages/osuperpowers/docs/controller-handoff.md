...
The discipline by which the orchestrator (executing-plans) drives the cdd engine. Cited by cli-driven-development, cli-task / cli-select, and the orchestrator skill. Cross-skill references use markdown links (semantic rule name + `#rule-<kebab>` anchor, e.g. `[Return Block](controller-handoff.md#rule-return-block)`).
