...
- **Exit**: `blocker=0?` routes to `fix-group` (both branches; the re-run path is determined by the entry edge)
...
- **Exit**: entered via blocker>0 → `run-group-review` (re-run); entered via blocker=0 → `more-groups?` (REVIEW_FIX closing round — no re-run after blocker=0)
