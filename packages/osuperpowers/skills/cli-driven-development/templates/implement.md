...
5. **`task` field is required and must be an integer** — write `"task": {{TASK}}` as a JSON integer (no quotes). Writing `"task": "{{TASK}}"` (string) causes schema validation failure.
...
8. **`artifacts` field is required** — always write `"artifacts": { "brief": "...", "report": "...", "test_evidence": "..." }`. Missing `artifacts` causes schema validation failure.
