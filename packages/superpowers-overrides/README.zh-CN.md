...
Token-efficient CDD 编排通过 plugin 内脚本 dispatch——`os-executing-plans` 编排，`cli-driven-development` 驱动 harness 链。Orchestrator 只解析一次 harness；唯一 CLI runner 是 `engineering/bin/engine/cdd-run.mjs`。
...
共享库：`engineering/bin/engine/lib/` Node 模块 — `runner.mjs`（workspace 路径契约 + task/plan run-loop `runTask` / `runPlan`）、`registry.mjs`（harness registry）、`contract.mjs`（commit gate + handoff）、`templates.mjs`（mode prompt）、`ledger.mjs`（ledger 追加）；`engineering/bin/engine/cdd-run.mjs` 是唯一 CLI runner（`--harness <name> --task N --mode M` | `--plan <path>`）。

**Mode A（单 task）：** `{engineering}/bin/engine/cdd-run.mjs --harness <name> --task N --mode implement|review|fix`

**Mode B（plan driver / AFK）：** `{engineering}/bin/engine/cdd-run.mjs --harness <name> --plan <path>` — pending tasks × 3-mode 链。
