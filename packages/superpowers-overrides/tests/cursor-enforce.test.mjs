...
const ENFORCE = fileURLToPath(
  new URL("../../superpowers-overrides/bin/cursor-enforce.mjs", import.meta.url),
);

const OVERRIDE = "engineering:os-brainstorming";
const SKILL_SUFFIX = "../engineering/skills/os-brainstorming/SKILL.md";
const TARGET_SKILL_PATH = "/repo/packages/engineering/skills/os-brainstorming/SKILL.md";
...
function writePending(key, detectedAt = Math.floor(Date.now() / 1000)) {
  const dir = join(pendingRoot, "oscaner-superpowers-overrides", "pending");
...
test("cursor-enforce: pending + Read of non-target path → deny", () => {
  ...
  try {
    ...
    const out = enforceTool("conv-e3", "Read", {
      path: "/repo/packages/engineering/skills/os-debugging/SKILL.md",
    });
    assert.equal(JSON.parse(out).permission, "deny");
...
test("cursor-enforce: pending + Skill(wrong target) → deny", () => {
  ...
  try {
    ...
    const out = enforceTool("conv-e4b", "Skill", { skill: "engineering:os-debugging" });
    assert.equal(JSON.parse(out).permission, "deny");
  } finally {
    teardown();
  }
