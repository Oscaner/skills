...
jq -e '.override == "engineering:os-brainstorming"' "$PENDING_ROOT/conv-test-2.json" >/dev/null
jq -e '.skill_suffix == "../engineering/skills/os-brainstorming/SKILL.md"' "$PENDING_ROOT/conv-test-2.json" >/dev/null
...
if [ -f "$repo_attach" ]; then
  ...
  jq -e '.override == "engineering:os-brainstorming"' "$PENDING_ROOT/conv-test-3.json" >/dev/null
fi
