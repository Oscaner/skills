...
const ROUTER = fileURLToPath(
  new URL("../../superpowers-overrides/bin/prompt-expansion.mjs", import.meta.url),
);
...
test("prompt-expansion: superpowers:brainstorming → Skill(engineering:os-brainstorming)", () => {
  const ctx = context(runCommand("superpowers:brainstorming"));
  assert.match(ctx, /MANDATORY OVERRIDE/);
  assert.match(ctx, /Skill\(engineering:os-brainstorming\)/);
});

test("prompt-expansion: bare /brainstorming → Skill(engineering:os-brainstorming)", () => {
  const ctx = context(run("/brainstorming"));
  assert.match(ctx, /Skill\(engineering:os-brainstorming\)/);
});

test("prompt-expansion: superpowers:writing-plans → Skill(engineering:os-writing-plans)", () => {
  const ctx = context(runCommand("superpowers:writing-plans"));
  assert.match(ctx, /Skill\(engineering:os-writing-plans\)/);
});

test("prompt-expansion: superpowers:subagent-driven-development → Skill(engineering:cli-driven-development)", () => {
  const ctx = context(runCommand("superpowers:subagent-driven-development"));
  assert.match(ctx, /Skill\(engineering:cli-driven-development\)/);
});
...
test("prompt-expansion: /using-git-worktrees → Skill(engineering:os-finishing) (shared target)", () => {
  const ctx = context(runCommand("/using-git-worktrees"));
  assert.match(ctx, /Skill\(engineering:os-finishing\)/);
});

test("prompt-expansion: /finishing-a-development-branch → Skill(engineering:os-finishing)", () => {
