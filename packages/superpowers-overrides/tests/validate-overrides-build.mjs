#!/usr/bin/env node
// packages/superpowers-overrides/tests/validate-overrides-build.mjs — Node port
...
const ROOT = path.resolve(HERE, ".."); // packages/superpowers-overrides
const REPO = path.resolve(ROOT, "../.."); // repo root
const ENGINE = path.join(REPO, "packages/engineering");
...
function main() {
  ...
  check("validate manifest target existence (cross-plugin)", () => {
    const m = loadManifest();
    for (const t of m.targets) {
      const [plugin, skill] = t.name.split(":");
      let p;
      if (plugin === "mattpocock-skills") {
        p = path.join(REPO, "vendors", plugin, "skills", "engineering", skill, "SKILL.md");
      } else {
        p = path.join(REPO, "packages", plugin, "skills", skill, "SKILL.md");
      }
      assert(existsSync(p), `missing target skill: ${p}`);
    }
  });
