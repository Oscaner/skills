...
All CLI scripts live in `plugins/engineering/bin/` (`cdd-run.sh` / `cdd-exec.sh` / `cdd-select.sh`); templates in `plugins/engineering/templates/cdd/`. Version syncs with plugin release. `{plugin_root}` resolution via `cdd_plugin_root`（`bin/lib/cdd-common.sh`）/ [cli-select](../skills/cli-select/SKILL.md).
...
The orchestrator PreToolUse gate（`plugins/engineering/bin/lib/cdd-orchestrator-gate.sh`，T3 迁入 engineering；p1-slim.2）blocks direct repo edits while a task is active. Judgment is one decision point — `cdd_gate_decide` resolves `active_ws` **once** (bound-ws first, scan only when unbound) and threads that same workspace through both phase and write checks.
