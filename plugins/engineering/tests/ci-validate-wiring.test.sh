#!/usr/bin/env bash
# ci-validate-wiring.test.sh — validate 脚本的 engineering 接线完整性守卫
#
# Guards the orchestrator (scripts/ci-validate.sh) so future edits cannot drop
# engineering coverage from `pnpm run validate`. Each engine test, the
...
marker="$(grep -nE '== 5b\. engineering plugin validation ==' "$VAL" | head -1 | cut -d: -f1 || true)"
[ -n "$marker" ] || fail "missing == 5b. engineering plugin validation == marker"

# 2. plugin.json structural check present (prints "OK — N engineering skills")
grep -q 'engineering plugin validation' "$VAL" || fail "5b echo missing"
grep -q 'engineering skills' "$VAL" || fail "plugin.json skills-count print missing"
...
for t in "${ENGINE_TESTS[@]}"; do
  n="$(grep -c "engineering/tests/$t" "$VAL" || true)"
  [ "$n" -eq 1 ] || fail "$t: expected 1 invocation in ci-validate.sh, got $n"
  ln="$(grep -n "engineering/tests/$t" "$VAL" | head -1 | cut -d: -f1 || true)"
