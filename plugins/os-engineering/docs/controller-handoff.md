...
编器（spor-sdd 过渡期 / os-executing-plans P2）驱动 cdd 引擎的纪律。被 cli-driven-development、cli-task / cli-select 与编器技能引用。跨技能引用用 markdown 链接（语义规则名 + `#rule-<kebab>` 锚，如 `[Return Block](controller-handoff.md#rule-return-block)`）。
...
handoff 写入内联在各模式模板（`templates/cdd/{implement,review,fix}.md` + `_handoff-write-fragment.md`），无独立 handoff 模式。编器不得自行 merge Standards/Spec prose。
