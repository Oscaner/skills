...
# rule-reference scan, and the migrated-engine zero-residue grep must be invoked
...
grep -q 'zero sdd residue in migrated engine' "$VAL" || fail "zero-residue OK echo missing"
grep -qF 'plugins/os-engineering/docs/cdd-reference.md' "$VAL" || fail "zero-residue grep misses cdd-reference.md"
