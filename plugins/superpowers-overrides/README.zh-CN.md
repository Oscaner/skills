...
对 [superpowers](https://github.com/obra/superpowers) + [os-engineering](../os-engineering/) 的**触发路由器**。本插件**不带任何技能体**——它拦截上游 superpowers 触发，路由到匹配的 **os-engineering** 编排器（`os-*` / `cli-*`）或 **mattpocock-skills** 委托（`tdd`）。个人 override 规则在 os-engineering 的 `os-*` 技能里——它们 Read 上游基线并叠加个人规则。

## 路由器做什么

调用 `/brainstorming`、`/writing-plans` 等 superpowers skill 时，路由器先触发，匹配目标加载：

- `os-engineering:os-*` — 流程编排器，Read 上游并叠加个人规则（如澄清问题 → `grilling`，spec review → fresh subagent passes）
- `os-engineering:cli-driven-development` / `cli-*` — cdd 引擎技能
...
3. **项目规则** — `os-init spor`（os-engineering）写入项目（`CLAUDE.md` 或 `.cursor/rules/superpowers-overrides.mdc`）；Cursor 上 hooks 未命中时为 fallback。

## 路由器目标

| 触发 | 目标 | 作用 |
|------|------|------|
| `/brainstorming` | `os-engineering:os-brainstorming` | discovery delegate 给 `grilling`；subagent spec review；大 scope 走 overall/phase |
| `/writing-plans` | `os-engineering:os-writing-plans` | 分段写 plan + review；tickets 发布到 `docs/superpowers/tickets/` |
| `/subagent-driven-development` | `os-engineering:cli-driven-development` | cdd 引擎——harness CLI 三模式链（implement/review/fix） |
| `/executing-plans` | `os-engineering:os-executing-plans` | 三模式编排器（in-session / subagent / cli） |
| `/finishing-a-development-branch` | `os-engineering:os-finishing` | 分支收尾 / PR；禁止 worktree；conventional commits |
| `/systematic-debugging` | `os-engineering:os-debugging` | 先有证据再提 fix；delegate 给 `diagnosing-bugs` |
| `/test-driven-development` | `mattpocock-skills:tdd` | 红-绿循环；seam 确认门在 os-engineering 模板 |
| `/verification-before-completion` | `os-engineering:os-verification` | 无验证证据不得声称完成 |
| `/receiving-code-review` | `os-engineering:os-code-review` | 不清楚的反馈 → `grilling`；fix → `tdd` |
| `/using-git-worktrees` | `os-engineering:os-finishing` | 拒绝创建 worktree（用户策略） |
...
1. 从 oscaner marketplace 安装 `superpowers`、`superpowers-overrides`、`os-engineering`、`mattpocock-skills`。
...
- 需要 inline 上下文时 attach **os-engineering `os-*`** skill 文件（如 `os-brainstorming/SKILL.md`）。
...
Token-efficient CDD 编排通过 plugin 内脚本 dispatch——`os-executing-plans` 编排，`cli-driven-development` 驱动 harness 链。Orchestrator 只解析一次 harness；唯一 CLI runner 是 `os-engineering/bin/cdd-run.sh`。
