...
from trigger_patterns import attach_path_regexes, target_skill_read_regexes
...
for t in targets:
    ...
    target_rows.append(
        {
            "name": t.name,
            "skill_suffix": target_skill_suffix(t),
            "attach_res": attach_res,
        }
    )

targets_json = json.dumps(target_rows)
read_res_json = json.dumps({t.name: target_skill_read_regexes(t.plugin, t.skill) for t in targets})
