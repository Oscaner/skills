...
Naming evolved across releases: v1 emit model → v2 `-overrides` suffix → v3 `spor-*` prefix → **v4 trigger router** (current). v3's `spor-*` skill bodies were deleted; **superpowers-overrides now ships no skill bodies** and routes to os-engineering targets. See [CHANGELOG.md](../CHANGELOG.md) entries `6.2.0-overrides.3` through `6.2.0-overrides.6`.
...
## Solution (v4 — trigger router + os-engineering)

**superpowers-overrides** is a **trigger router** (claude + cursor), not a skill pack:

1. **No skill bodies** — `skills/` is absent/empty; `overrides.manifest.json` maps upstream triggers to targets in **os-engineering** (`os-*` / `cli-*`) or **mattpocock-skills** (`tdd`).
...
4. **os-engineering multi-harness emit** — the skills + engine plugin emits thin per-harness manifests (claude/cursor/codex/kimi/gemini/pi) all pointing at the canonical `./skills/` tree, plus a shared `.agents/skills/` copy (os-engineering + upstream superpowers) for codex/gemini/pi/qoder/opencode scanners. Modeled after impeccable's build.js + PROVIDERS pattern.
...
Cross-harness PreToolUse enforcement for CDD orchestrator sessions (Cursor + Claude Code). The gate ships with **os-engineering** — the overrides plugin is a trigger router and carries no gate hooks.
...
3. `^/os-<upstream-slug>` — e.g. `/os-brainstorming` (os-engineering targets)

All invoke `bin/override-prompt-expansion.sh`, which injects `additionalContext` containing **MANDATORY OVERRIDE** and the required `Skill(<target-name>)` first call (e.g. `Skill(os-engineering:os-brainstorming)`).

Project `CLAUDE.md` self-check (from `os-init spor`) is fallback when hooks are unavailable.

### Self-check rules (both harnesses)

`os-init spor` (os-engineering) writes committed generator output into the project:
...
- **Anti-pattern:** manually attach upstream `superpowers/*/SKILL.md` body — use slash commands or read the target os-engineering skill instead; upstream SKILL full text in context still requires the target skill first.
...
```json
{
  ...
  "targets": [
    {
      "name": "os-engineering:os-brainstorming",
      "overrides": "superpowers:brainstorming",
      "source": "../os-engineering/skills/os-brainstorming"
    }
  ]
}
```
...
| `name` | Target skill id in all harnesses (`plugin:skill`, e.g. `os-engineering:os-brainstorming`) |
...
The router ships no `spor-*` skills. Target skill ids are the os-engineering / mattpocock canonical names (`os-engineering:os-*`, `os-engineering:cli-*`, `mattpocock-skills:tdd`). Init entry point: `os-init spor` (Claude Code: `/os-init spor`).
...
Regenerate after editing `overrides.manifest.json`, os-engineering skills, or generator templates.

## Plugin discovery fallback (Cursor)

Skills ship under `plugins/os-engineering/skills/` in the plugin tree. After marketplace install, verify the os-engineering skills (12 emitters + os-init) appear in the agent skills list.

If skills are missing (Team Marketplace blocked or third-party import disabled):

```bash
mkdir -p .cursor/skills
cp -R path/to/plugins/os-engineering/skills/* .cursor/skills/
...
Templates: `os-engineering/templates/cdd/` (implement, review, fix) + `_handoff-write-fragment.md`.
...
4. **Init** — copy or refresh committed `build/generated/*` at runtime; never run generators in init. Generated self-check files embed `os-engineering-version` (Cursor frontmatter / Claude HTML comment) stamped from the os-engineering plugin version; `os-init spor` compares project rules against installed version and overwrites when missing or stale.
