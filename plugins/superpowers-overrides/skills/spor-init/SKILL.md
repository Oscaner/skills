---
name: spor-init
description: Initialize superpowers-overrides wiring for the current project. Run once per project — Claude Code writes the override self-check to CLAUDE.md; Cursor writes .cursor/rules/superpowers-overrides.mdc. Trigger on `/spor-init` or `/superpowers-overrides:spor-init`. After running, superpowers skills automatically trigger their overrides without global ~/.claude configuration.
---
...
2. Locate plugin root: derive from this skill's `fullPath` in `<agent_skills>` (strip `/skills/spor-init/SKILL.md` suffix), or use the in-repo path `plugins/superpowers-overrides/` when working in the marketplace clone.
...
4. Report the exact path written. Remind the user:
   ...
   - Use `/spor-bs` (etc.) or rely on rules intercept for upstream triggers.
...
3. Find the right insertion point — read the existing file first:
   - If the file **does not exist**: stop. Tell the user to run the project's base `/init` first to generate `CLAUDE.md`, then re-run `/superpowers-overrides:spor-init`.
