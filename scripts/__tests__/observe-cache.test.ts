...
// `/cost` / `--debug` harness output into { readTokens, writeTokens } (CacheObserver#extractUsage —
// Task 9 class face). Pure, harness-shaped-text driven; the actual measurement run is a documented
// dev-side action (no live harness in CI). Also pins the argv parser (boolean-presence semantics)
// and the measurement-mode cross-phase derivation (review/fix fixed-point from real prior handoffs).

import { mkdtempSync, rmSync, writeFileSync } from "node:fs";
import { tmpdir } from "node:os";
import path from "node:path";
import { afterEach, beforeEach, describe, expect, it } from "vitest";
import { cacheObserver } from "../observe-cache.ts";
...
describe("extractCacheUsage — parse prompt-cache read/write tokens from harness output", () => {
  it("Anthropic --debug key=val form (cache_creation_input_tokens / cache_read_input_tokens)", () => {
    ...
    expect(cacheObserver.extractUsage(log)).toEqual({ readTokens: 6093, writeTokens: 3482 });
  });
  ...
  it("OpenAI-verbose prompt_cache_read/write_tokens form", () => {
    ...
    expect(cacheObserver.extractUsage(log)).toEqual({ readTokens: 300, writeTokens: 200 });
  });
  ...
  it("human /cost table lines (thousands separators, case-insensitive)", () => {
    ...
    expect(cacheObserver.extractUsage(log)).toEqual({ readTokens: 6093, writeTokens: 2000 });
  });
  ...
  it("takes the last summary occurrence (round-after-round capture)", () => {
    ...
    expect(cacheObserver.extractUsage(log)).toEqual({ readTokens: 1234, writeTokens: 100 });
  });
  ...
  it("zero-token cache fields are still a measurement (not null)", () => {
    ...
    expect(cacheObserver.extractUsage(log)).toEqual({ readTokens: 0, writeTokens: 0 });
  });

  it("no cache fields → null (not a measurable round)", () => {
    expect(cacheObserver.extractUsage("status: APPROVED\ncommits: base=x head=y")).toBeNull();
    expect(cacheObserver.extractUsage("")).toBeNull();
  });
});

describe("parseArgs — presence-based booleans + value-taking options", () => {
  it("defaults: --debug flag, claude harness, 2 rounds", () => {
    expect(cacheObserver.parseArgs(["--", "ws", "7", "implement"])).toEqual({
      harness: "claude",
      rounds: 2,
      flag: "--debug",
      workspace: "ws",
  ...
  it("boolean flags are presence-only and never swallow the workspace positional (natural usage without `--`)", () => {
    expect(cacheObserver.parseArgs(["--debug", "ws", "7", "implement"]).flag).toBe("--debug");
    expect(cacheObserver.parseArgs(["--debug", "ws", "7", "implement"]).workspace).toBe("ws");
    expect(cacheObserver.parseArgs(["--debug", "ws", "7", "implement"]).mode).toBe("implement");
  });

  it("--cost stays an explicit opt-in; --harness/--rounds keep consuming their value", () => {
    const a = cacheObserver.parseArgs(["--cost", "ws", "7", "fix"]);
  ...
  it("rejects a zero or non-numeric task (citty enforces presence only, not the integer boundary)", () => {
    expect(() => cacheObserver.parseArgs(["--", "ws", "0", "implement"])).toThrow(
      /positive integer/,
    );
    expect(() => cacheObserver.parseArgs(["--", "ws", "abc", "implement"])).toThrow(
      /positive integer/,
    );
  });
});
...
describe("priorHandoffPaths — measurement-mode cross-phase derivation (mirrors dispatch/task.ts)", () => {
  ...
  it("implement: no findings, no fixed point (byte-identical rounds)", () => {
    expect(
      cacheObserver.priorHandoffPaths({ workspace: ws, task: 7, mode: "implement", round: 1 }),
    ).toEqual({
      findingsPath: "",
      fixedPoint: "",
    });
    expect(
      cacheObserver.priorHandoffPaths({ workspace: ws, task: 7, mode: "implement", round: 2 })
        .fixedPoint,
    ).toBe("");
  });

  it("fix round R: findings + fixed point come from the same-round review handoff (prev = review.task:R)", () => {
    const r1 = cacheObserver.priorHandoffPaths({ workspace: ws, task: 7, mode: "fix", round: 1 });
  ...
  it("missing prior review handoff → the review path as findings, empty fixed point (documented approximation)", () => {
    const r2 = cacheObserver.priorHandoffPaths({ workspace: ws, task: 7, mode: "fix", round: 2 });
  ...
  it("review round 1: fixed point from the implement handoff; review round R: from fix-(R-1)", () => {
    expect(
      cacheObserver.priorHandoffPaths({ workspace: ws, task: 7, mode: "review", round: 1 })
        .fixedPoint,
    ).toBe("a".repeat(40));
    expect(
      cacheObserver.priorHandoffPaths({ workspace: ws, task: 7, mode: "review", round: 2 })
        .fixedPoint,
    ).toBe("e".repeat(40));
  });
});
