...
function kimiInstructions(plugin) {
  return `Kimi Code tool mapping for ${plugin.name} skills:\n\n` +
    `- The cli-* family (cli-select, cli-driven-development) orchestrate work through the cdd engine's CLI. When a skill says to run an orchestrator, select a task, or drive a development loop, call the cdd-* scripts from the plugin's bin/ via the terminal tool.\n` +
